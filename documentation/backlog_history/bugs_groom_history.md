...
## Archived 2026-08-15 (bugs.md #548): the status paragraph as it read before #548 closed
...
## Archived 2026-08-14 (bugs.md #547): the "Zero open Blocker or Major bugs" checklist box, as it read before being unchecked

- [x] Zero open Blocker or Major bugs in the Ranked Backlog — **Re-checked 2026-08-13 while shipping Application Knowledge:** still zero open, but the box is only checked because #544 was fixed in the same task that found it. #544 was graded **Major** on the existing rubric (it put a qualification the operator does not have onto a real employer's screening question, silently, with nothing in the fill summary to show it) rather than downgraded because it was cheap to fix. It had been live since the vault shipped and was found by reading `pkg/answers/patterns.go`, not by any test — the test suite pinned the classification as correct and never checked what got auto-filled. The four open rows remain #524, #526, #528 and #532, all Minor. **Prior — Re-checked 2026-08-08 after closing #534:** still zero. The four open rows are #524, #526, #528 and #532, all Minor. **Prior — Checked 2026-08-06:** the acceptance trial's last Major closed. #521 (indistinguishable duplicate cards, which caused a false APPLIED record) closed 2026-08-06; #520 (Lever submissions fail in the assisted browser) and #519 (assisted prefill non-functional on Greenhouse/Lever) closed the same day. Only Minor rows (#522-#525) remain open. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*

Superseded 2026-08-14 (production-evidence audit of the first two real applications; #546 closed, #547/#548 filed):
...
Superseded 2026-08-14 (bugs.md #545 closed):
...
**Prior — MET 2026-08-08 (#534's tracker durable-checkpoint close), superseded 2026-08-11 by #535's find-and-fix.** Re-verified 2026-08-08: zero open Blocker or Major rows. **#534** (`StartTracker` only ever fetched the newest ~51 messages, so downtime longer than ~4 hours of this inbox's traffic put outcome mail permanently out of reach) is fixed: the tracker now tracks a durable IMAP UID checkpoint (`tracker_cursor`) with an independent, bounded historical catch-up range seeded from live evidence rather than a hardcoded lookback — see ADR-004 and `documentation/backlog_history/bugs_done_details.md` item #534 for the full design and production validation. A second defect was found and fixed during that same validation, before it ever shipped: a gap in IMAP UID numbering (deleted mail) could stall catch-up forever under the first version of the fix — confirmed live, then fixed and re-verified live before the task was considered done. The fixed binary is deployed and running in production as of this close: bootstrap found a real ~2009-UID historical catch-up window, three deliberate restarts all resumed correctly from the persisted checkpoint, `processed_emails`/`unmatched_outcomes` counts stayed idempotent across repeated passes over already-covered mail, and a single tracker process remained running throughout. No matched (REJECTED/INTERVIEW_REQUESTED) outcomes were recovered within this session's observation window — catch-up had progressed roughly 30% through its historical span by the end of the session, entirely over territory the pre-#534 tracker had already covered — so usable matched-outcome data is still 0; the fix guarantees eventual coverage across future scans, not instant coverage in one sitting, and the tracker was left running to keep draining on its own 15-minute cadence. `go build ./...`, `go vet ./...`, `go test ./...` (including 10 new `pkg/tracker` tests run against a real in-process IMAP server, plus all 22 pre-existing ones), and `gofmt -l ./cmd ./pkg ./internal` all pass.

**Prior — zero-Blocker/Major box, re-checked 2026-08-07 after filing #534, superseded 2026-08-08 by #534's close.** Still zero. #534 was deliberately filed `Minor` and the reasoning was written down in its Details section so a later session could challenge it rather than guess: it destroys real outcome data, but only after the tracker has been stopped longer than its fetch window, and the tracker is correct in steady state — it did not block anyone from using the product at the time. It was not graded down to protect this box. The six open rows were #522, #524, #526, #528, #532 and #534, all Minor.

**Prior — MET 2026-08-07 (evening, first production validation of #529/#533), superseded 2026-08-08 by #534's close.** Re-verified 2026-08-07 (evening): zero open Blocker or Major rows. The tracker was restarted 2026-08-07 15:47 local after a 14-day stop and ran nine consecutive scans with zero errors — no `Error during scan`, no failed persist, no failed acknowledgement, no warnings, no authentication loop, no database-lock loop; one process, built from `aaebb63` (code-identical to HEAD, the two later commits being `.gitignore`-only), 2.2 CPU-seconds over two hours, sleeping on its normal 15-minute cadence. #533 worked in production: 120 messages acknowledged and 19 outcome-shaped emails recorded in `unmatched_outcomes` that the pre-fix code would have acknowledged and destroyed — 12 distinct sender domains, all interview-shaped, max stored field lengths 19 and 27 characters, so no email content leaked into the table. Every #533 invariant was checked against live data rather than assumed: zero unmatched rows lack their `processed_emails` acknowledgement (the atomicity guarantee), zero message IDs appear twice, and two fully-observed scans re-fetched the same window and wrote nothing at all — idempotency proven without touching the inbox. One ambiguous outcome was exercised and correctly refused to pick an arbitrary application. #529 was *not* exercised — no outcome matched any application, so no stage event was written. Unrelated mail modified nothing: zero `job_funnel` rows and zero stage events were written across all nine scans, and the 7 confirmed applications kept their original timestamps. The validation also found one new defect, filed as #534: `StartTracker` only ever fetches the newest ~51 messages, which at this inbox's measured ~12.5 messages/hour is about 4 hours of coverage, so the 14-day outage put every outcome older than that permanently out of reach — which is why usable outcome data was still 0 matched outcomes. `go build ./...`, `go vet ./...`, `go test ./...` (including all 22 `pkg/tracker` tests), and `gofmt -l ./cmd ./pkg ./internal` all passed.

**Prior — `cmd/tracker` gate-box note, superseded 2026-08-07 by the production validation.** Verified 2026-07-22 00:05: with a freshly generated Google App Password, a full inbox scan completed cleanly ("Scan complete", no crash), detecting a real rejection (Glimpse) and a real interview invitation. The scan also exposed serious classification false positives, filed separately as bug #20 — the gate box is about crash-safety and it passes; classification quality is its own bug. Earlier attempt note (2026-07-21): built and ran one cycle in the container. The crash-safety half is verified — Google rejected the login and the tracker handled it cleanly (logged the error, proceeded to its sleep loop, no crash). The credentials half is blocked on the user: `.env`'s `IMAP_APP_PASSWORD` is 12 characters, but Google app passwords are 16 — it's malformed or stale, and Google returned "Application-specific password required". Needs a freshly generated Google App Password (Google Account → Security → 2-Step Verification → App passwords) before a genuine logged-in scan can be verified. **Note added 2026-08-07:** the "real rejection (Glimpse)" claim in that note was later shown to be a false positive written by the pre-#20 binary, whose database writes were guaranteed no-ops — see #529's archived account in `bugs_done_details.md`.

**Prior — MET 2026-08-07 (after closing #529 and #533, before the tracker was validated in production).** The gate is now met: no open Blocker or Major rows remain. Re-verified 2026-08-07 after closing #529 and #533: still zero open Blocker or Major rows. The five open rows are #522, #524, #526, #528 and #532, all Minor. **#533 closed this day and was sequenced deliberately: it is the defect that made restarting the tracker destructive.** An outcome email matching no application was acknowledged and discarded in the same transaction, and `StartTracker` never revisits an acknowledged Message-ID — so with 44 hand-off rows against 7 confirmed applications, and two weeks of unread mail waiting, the first scan would have destroyed exactly the evidence the outcome loop exists to collect. Lost outcomes are now recorded in `unmatched_outcomes` (status, sender domain, timestamp — no email content) and reported as bounded counts at the end of each scan, while the message is still acknowledged so bug #20's rescan churn does not return. **A standing figure was also corrected:** this file and `improvements.md` both cited "58 confirmed applications". 51 of those `applied_jobs` rows predate bug #94's 2026-07-25 fix and record *document generation*, not submission. **The true confirmed count is 7.** `applied_jobs` and `job_funnel` do not contradict each other, no reconciliation is owed, and zero stale dedup rows currently suppress work. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*

**Prior — MET 2026-08-06 (`/groom_backlogs` re-verification).** #521, the last one, closed 2026-08-06 — the Assisted Apply queue can no longer render two postings of the same role at the same company as indistinguishable cards, and the dialog that writes the irreversible `APPLIED` record now names the job and warns when a sibling could be confused with it. Verified live against the real `applications.db`: 517 of 524 queued rows carry a requisition identifier, 130 rows across 49 duplicate groups are all warned, and no duplicate group is indistinguishable without being flagged as such. Minor rows remain open and are ranked against `improvements.md`'s Pending rows in the normal way rather than ahead of them. **Re-verified 2026-08-06 (`/groom_backlogs`):** still zero open Blocker or Major rows; the six open rows are #522, #524, #526, #527, #528 and #529, all Minor. #525 closed the same day, and #527/#528/#529 were filed from that work and this groom. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass, as do 20 of 20 dashboard UI tests. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*

**Prior — UNMET 2026-08-06 (after #520).** The gate remained unmet on one open Major defect, down from two: #520 closed 2026-08-06. Lever postings no longer route into the assisted browser at all. Career Agent deliberately does not try to disguise that browser to get past Lever's verification; instead a Lever row now shows "Finish in your own browser" with a real link to the posting, keeps the prepared documents and the "I saw a confirmation — Mark Applied" path, and no route — dashboard, CLI, or batch — can spawn a browser whose submission Lever is already known to refuse. Still open at that point: #521, a data-integrity defect that already produced one false `APPLIED` record; it was the only thing between this project and a met gate. Four Minor rows (#522-#525) remained. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all passed, as did 20 of 20 dashboard UI tests.

**Prior — UNMET 2026-08-06 (after #519).** The gate remained unmet on two open Major defects, down from three: #519 closed 2026-08-06. Assisted Apply now prefills through the same dedicated Greenhouse/Lever/Ashby handlers the automatic path uses, verified live against a real Greenhouse posting and a real Lever posting — fields, location, country, résumé, and cover letter landed on the form, the URL never changed, and nothing was submitted. That live run also found and fixed a second defect affecting the automatic path too: the modern Greenhouse board names its upload control `id="resume"` with no `name`, so the résumé had been silently going unattached. Still open at that point: #521, a data-integrity defect that already produced one false `APPLIED` record, and #520, which left Lever submissions failing inside the assisted browser (#519 makes that form arrive prefilled but does not change whether Lever accepts it). Four Minor rows (#522-#525) remained. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all passed.

**Prior — MET 2026-08-05 (Assisted Apply acceptance trial, second candidate).** There are no Pending bug rows and the zero-Blocker/Major box remains checked. The trial resumed after #515 and was **halted a second time without submitting any application**. Candidate job 259800 (jobgether, "AI Automation Engineer", fit score 100) passed every documents and safety gate and opened a real, verified application in the assisted browser — and the employer page showed the role was scoped to **India**, which the user does not want. That exposed #516, a Blocker: discovery had no geographic filter at all, `job_location`/`is_remote` were empty for all 12,902 rows, and `remote_only` says nothing about where remote work may be performed from. #516 was fixed and closed in the same session and verified end to end against the live Lever feed (1,952 of 2,993 postings correctly rejected), so the zero-Blocker/Major box stays checked on a fixed tree. The lease released cleanly, nothing was marked applied, and no duplicate record was created. Safety gates re-confirmed on fresh evidence: effective mode assisted, minimum fit 70, scoring active, and automatic submit click inactive — forced off by `ApplyOperatorSettings` despite `profile.yaml` carrying `auto_submit_click: true` — with a live daemon that rewrote `active_operator_settings.json` at startup. Applications confirmed: still 0 of 5. Four non-blocking observations recorded rather than fixed: `/api/agent/stop` can report success while the daemon lives; `daemon_active` reads true with no agent running; the dashboard launches Assisted Apply through a `go run` wrapper, so the built `career_assist_bin` is never what executes; and `installAssistedContextGuard` aborts requests silently with no log line, which cost real diagnostic time. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*

**Prior — MET 2026-08-05 (Assisted Apply acceptance trial, first candidate).** There are no Pending bug rows and the zero-Blocker/Major box remains checked. The trial ran inside the `career-agent` distrobox against container-built binaries and was **halted at its first candidate without submitting any application**: verifying the "documents are correct" precondition exposed #515, a Blocker in which Assisted Apply would have uploaded a 116-byte placeholder note in place of the user's résumé. #515 was reproduced against a scratch database, fixed, and closed in the same session, so the box is checked on a fixed tree rather than an unexamined one. The trial's other gates all passed on fresh evidence: effective mode assisted, minimum fit 70, scoring active, automatic submit click inactive, and a live daemon that re-adopted those settings at startup. Two non-blocking observations were recorded rather than fixed — the daemon did not exit on SIGTERM and needed SIGKILL while `/api/agent/stop` reported success, and `daemon_active` is computed from a settings file with no liveness check, so it reads true when no agent is running. Applications confirmed: 0 of 5; the trial has not yet measured a real submission. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` pass.
...
**Prior — 2026-08-01, session forty-four.** Bug #490 is Done: both automatic confirmation and manual handoff promotion now persist one canonical UTC `job_funnel.applied_at` timestamp, while later status transitions preserve it. Focused storage regression coverage plus the full build, vet, test, and formatting loop are clean. The highest remaining score is a three-way 1.5 tie (#504, #503, and improvement #491). See `documentation/backlog_history/bugs_done_details.md` item #490 for the full account.

**Prior — 2026-08-01, session forty-three.** Bug #480 is Done: every retryable pipeline path now supplies its causal error to `UpdateFunnelStatusRetryable`, which persists the final error in `status_reason` when a row reaches `RETRY_EXHAUSTED`. The complete backoff/exhaustion sequence and terminal reason are regression-tested; build, vet, full tests, and formatting are clean. The production daemon was rebuilt and restarted, so new exhausted rows carry their reason. #490 (1.5) is now the highest-scoring Pending bug, tied with improvement #491. See `documentation/backlog_history/bugs_done_details.md` item #480 for the full account.

**Prior — 2026-08-01, session forty-two.** Bug #501 is Done: a sanitized read-only measurement of the first post-#489 cohort found `QUARANTINED_PROMPT_INJECTION` fell from the 51.0% lifetime baseline to 0/8 rows, with no post-fix audit-CSV detections. This is not an outcome-success claim: all 8 rows instead reached `FAILED_SUBMIT` (100% vs. 4.5% lifetime), so the cohort is both too small and materially offset by a downstream failure mode. Filed #507 to diagnose that failure before treating #489 as live-proven. #480 (1.67) remained the highest-scoring Pending bug. See `documentation/backlog_history/bugs_done_details.md` item #501 for the full account.

## Usability Gate — prior status paragraphs (newest first, as originally written)

**Prior — UNMET 2026-08-06, three open Majors (superseded when #519 closed the same day).** The gate is now unmet because the trial exposed three open Major defects (#519, #520, #521) that were filed rather than fixed, at the operator's instruction. The most consequential is #519: Assisted Apply cannot prefill on Greenhouse or Lever, so all five applications were typed by hand despite the pipeline having already filled each form successfully — the feature's core promise is unmet on every application. #521 is a data-integrity defect that already produced one false `APPLIED` record. #520 leaves roughly half the ATS market unusable through the assisted browser. Four Minor rows (#522-#525) were filed alongside them. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(The zero-Blocker/Major checklist box carried the matching three-Major note and was replaced by the two-Major one at the same time.)*
...
**Prior — 2026-08-01, session forty-five.** Bug #503 is Done: `TwoStepVerification` now sends structured quarantine threats to the same CSV logger as the other security boundaries, preserving the rejection and recording the URL with no fabricated company attribution. Focused submitter regression coverage plus the full build, vet, test, and formatting loop are clean. The highest remaining score is improvement #491 (1.5), followed by bugs #482 and #507 (1.33). See `documentation/backlog_history/bugs_done_details.md` item #503 for the full account.
**Prior — 2026-08-01, session forty-seven.** Bug #507 is Done: the eight post-#489 `FAILED_SUBMIT` rows were all sanitized-log `target closed` failures before page validation or document generation. Initial browser setup now recreates the context once on that exact Playwright failure; the regression test and full build, vet, test, and formatting loop are clean. Bug #482 is now the highest-scoring Pending bug (1.33). See `documentation/backlog_history/bugs_done_details.md` item #507 for the full account.
**Prior — 2026-08-02 groom.** There are no Pending bug rows and the zero-Blocker/Major box remains checked. The live dashboard now reports four eligible jobs, two applications, and zero interviews or rejections; this supersedes the prior zero-queue snapshot without exposing a reproducible defect. The outcome sample remains too small for improvement #493; #485 is the next free, above-floor enhancement. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` pass. *(Superseded 2026-08-05 by the Assisted Apply acceptance trial, which halted at bug #515 before submitting any application.)*

## Superseded status paragraph (archived 2026-08-07, on closing #529)

**The gate is now met: no open Blocker or Major rows remain.** Re-verified 2026-08-07 after closing #527: still zero open Blocker or Major rows. The five open rows are #522, #524, #526, #528 and #529, all Minor, and they are ranked against `improvements.md`'s Pending rows in the normal way rather than ahead of them. #527 closed this day — `pkg/storage`'s test harness no longer opens a `:memory:` database, so a query issued from inside an open result set reaches a real schema instead of failing `no such table`, and the assisted queue's `resume_ready` / `cover_letter_ready` fields are covered at the projection level for the first time rather than being asserted against the failure path. No production behaviour changed. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*

## Superseded status paragraph (archived 2026-08-07, on closing #533)

**The gate is now met: no open Blocker or Major rows remain.** Re-verified 2026-08-07 after closing #529: still zero open Blocker or Major rows. The five open rows are #522, #524, #526, #528 and #532, all Minor, and they are ranked against `improvements.md`'s Pending rows in the normal way rather than ahead of them. **#529 closed this day, and its headline was not a defect.** The tracker's 49 `processed_emails` rows are two scans (41 on 2026-07-22, 8 on 2026-07-24), not 49 outcome emails, and the tracker has not run since — `processed_emails` counts messages *looked at* within a fixed 50-message fetch window. The "real rejection" the Usability Gate credited to the 2026-07-22 scan was written by the pre-#20 binary 18 minutes before #20's fix landed, at a time when `cmd/tracker` never called `InitDB` and every database write was a guaranteed no-op; the artifact's own LLM-extracted field says the message was not a rejection at all. Zero outcomes is the correct state. Diagnosis did surface a genuine defect on the same path and it was fixed: the tracker's outcome write set `status` alone, so the stage-ledger trigger — which reads `NEW.last_updated` and `NEW.status_reason` — recorded every outcome backdated to the application's submission time and labelled with the submission's own reason code. It now stamps all three columns with a bounded outcome reason. **#532 was filed this pass** for the wider trigger defect found underneath it: `stage_duration_ms` is NULL for all 1,385 live ledger rows. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*

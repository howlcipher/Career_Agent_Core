...
- **Live-verified end to end**, not just by test: rebuilt and restarted the running dashboard binary (no live assisted browser was open at the time; the agent daemon was left untouched, since this fix is dashboard-only queue-projection logic it never calls). Wurl's card regained "I saw a confirmation — Mark Applied", the operator clicked it for the real prior submission, and `ConfirmAssistedSubmission` committed atomically: `assisted_state` → `completed` (`confirmation_provenance = manual_user_confirmation`), `job_funnel.status` → `APPLIED`, `dogfood_cohort_applications` ordinal 4 for job 1240. The dogfood cohort advanced from 3/5 to 4/5 with no duplicate row and no other job's state touched.

...
**MET 2026-08-17, on closing #551.** #551 was the sole open Major: `cmd/agent`'s `AttemptSubmit` filled real employer forms and recorded nothing about it, so the Assisted card had no way to describe its own agent's work once a copilot-mode fill reached `AWAITING_REVIEW`. Fixed by recording a real fill attempt — `storage.RecordAutomaticFillAttempt`, called only past every guard capable of ending `AttemptSubmit` before a control is touched (dead posting, bot protection, DOM quarantine, account-gated ATS), mirroring `FillAssistedMappedPage`'s `OnFormReached` discipline on the automatic side — and by generalizing `assisted_fill_summary` with a `fill_source` column so the card can tell a closed automatic browser's stale history apart from a live assisted one, rather than rendering evaporated field counts as though they were still on screen. See `documentation/backlog_history/bugs_done_details.md` item #551 for the full account. Zero other open Blocker or Major rows remain: five Pending rows, all Minor (#552, #553, #532, #526, #528). `go build ./...`, `go vet ./...`, `go test ./...` (including new `pkg/storage`, `pkg/submitter` and dashboard UI coverage), and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
- [x] Zero open Blocker or Major bugs in the Ranked Backlog — **Checked 2026-08-17, verified against the actual Pending table rather than a prose count.** #551 is Done: `cmd/agent`'s automatic fills now write real, guard-respecting provenance instead of nothing, and the card no longer describes a fill that evaporated as though it were current. Enumerated at close: **5 Pending rows, all Minor (#552, #553, #532, #526, #528)** — zero Blocker, zero Major. This is the third time this specific box has been unchecked and rechecked by a review finding a truthfulness defect in the fix meant to close the previous one (#535, then #548→#551); the lesson already on record — run the reviewers before believing a gate box — was applied here too, via this task's own independent read-only review pass before closing #551 (see `documentation/backlog_history/bugs_done_details.md` item #551 for its findings). *(prior verification history, including the 2026-08-15, 2026-08-14, 2026-08-13 and 2026-08-08 re-checks, archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 551 | [Automatic Apply fills real employer forms and records nothing, so the card cannot describe its own agent's work](#551-automatic-apply-fills-real-employer-forms-and-records-nothing-so-the-card-cannot-describe-its-own-agents-work) | Major | Done (2026-08-17) | — | deep-reasoning | See `documentation/backlog_history/bugs_done_details.md` item #551 for the full account. `cmd/agent`'s `AttemptSubmit` now records a real, guard-respecting fill attempt via a new `RecordAutomaticFillAttempt`/`fill_source` column, and the card distinguishes a closed automatic browser's stale history from a live assisted one. |
...
| 552 | [The review clock starts when an application was prepared, so every measurement exceeds the credibility cap and is silently discarded](#552-the-review-clock-starts-when-an-application-was-prepared-so-every-measurement-exceeds-the-credibility-cap-and-is-silently-discarded) | Minor | Done (2026-08-18) | — | standard | Fixed: `assistedReviewStartedAt` now reads `storage.GetApplySessionItemStartedAt` (session item `started_at`) rather than `assisted_fill_summary.recorded_at`. |
...
| 557 | [A revalidated legacy (CAPTCHA/manual-account) handoff loses its confirmation control once the browser closes, even after a genuine submit](#557-a-revalidated-legacy-captchamanual-account-handoff-loses-its-confirmation-control-once-the-browser-closes-even-after-a-genuine-submit) | Minor | Done (2026-08-20) | — | standard | Found live during the controlled dogfood run (application #4, Wurl, job 1240, `BLOCKED_CAPTCHA`-origin). `actionForRevalidation`'s `application_ready` case gated `RequiresExplicitSubmit` on `status == "AWAITING_REVIEW"` literally, so a `BLOCKED_CAPTCHA`/`MANUAL_REQUIRED`-origin job that skipped the guided Continue step and was submitted directly in the assisted browser had no "I saw a confirmation — Mark Applied" control left once the browser closed. Fixed by gating on `isAssistedEligibleStatus(status)` instead — the same predicate `ConfirmAssistedSubmission` itself already uses — so the UI affordance can never be narrower than what the backend will actually accept. See `documentation/backlog_history/bugs_done_details.md` item #557 for the full account. |
...
Fixed 2026-08-17 — full account in `documentation/backlog_history/bugs_done_details.md` item #551.
...
**Done (2026-08-18):** `assistedReviewStartedAt` now queries `storage.GetApplySessionItemStartedAt(db, jobID)` which reads `started_at` from `application_session_items` when the application is actively opened/in-progress in a session, returning zero time when unknown or not part of an active session. Verified with `go test ./pkg/storage ./cmd/dashboard` and `slopslint check --classify --enforce`.

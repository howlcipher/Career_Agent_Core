...
**MET 2026-08-11, on a tree with a Major found and fixed within the same task.** A safety-hardening task found and closed **#535** in the same session, before ever committing: `config.GetEffectiveSettings` and, more severely, `config.ApplyOperatorSettings` (the function that actually drives `profile.AutoSubmitClick`, consumed directly by `cmd/agent`'s startup path and the final `submitter.AttemptSubmit` click) both fell back to inferring Automatic mode from legacy `profile.yaml` booleans whenever `applications/operator_settings.yaml` was missing — graded **Major** per the existing rubric (irreversible external-action risk) rather than downgraded to protect this box, and the box is only checked because it was fixed, not because it was found. The fix (`config.DefaultOperatorSettings()` as the single authoritative fail-closed fallback, consumed by every call site through `ApplyOperatorSettings`) means Automatic mode now requires an explicit, successfully loaded `operator_settings.yaml` — a missing or invalid settings file can no longer resolve to Automatic under any legacy `profile.yaml` combination. See `documentation/backlog_history/bugs_done_details.md` item #535 for the full account. Zero other open Blocker or Major rows remain (#534's tracker durable-checkpoint close carries over from before this task; full account in the same file). `go build ./...`, `go vet ./...`, `go test ./...` (including 12 new `pkg/config` tests), and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
- [ ] Zero open Blocker or Major bugs in the Ranked Backlog — **Unchecked 2026-08-15, verified against the actual Pending table rather than a prose count.** #548 is Done, which closed the Major this box was unchecked for. It does not get checked, because #548's own independent review round filed **#551, which is Major and open**: `cmd/agent` fills real employer forms, reaches `AWAITING_REVIEW`, logs *"form filled — awaiting human review"*, and writes no fill record at all — so all 24 queued `AWAITING_REVIEW` rows arrived through a path that reports nothing about its own work. Enumerated at close: **8 Pending rows, 1 Major (#551) and 7 Minor (#524, #526, #528, #549, #552, #553 and, ranked among them, nothing else)**. Two things worth writing down. First, the box was unchecked *by the review of the fix that was supposed to check it*, which is the second consecutive time this has happened and is an argument for running the reviewers before believing a gate box. Second, **Major here is a truthfulness grade, not a stop-work signal**: #551 costs the operator no application and no privacy, because a copilot fill's browser has closed by the time they open the form and its typed values are gone either way — so the honest reading is that this box is unchecked *and* the product is usable. A session that re-grades #551 to Minor on that reasoning would not be protecting this box, but it should say so out loud rather than edit the cell quietly. *(prior verification history, including the 2026-08-14, 2026-08-13 and 2026-08-08 re-checks, archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
**2026-08-15, #548 closed; #551/#552/#553 filed by its review round.** The Major that unchecked the Usability Gate is fixed: preparing an application no longer stamps a fill outcome, and the card no longer reports a fill that never ran. The evidence that mattered was durable state read against the code, not the test suite — **all 11 `assisted_fill_summary` rows paired 1:1 with an `inspected` preflight verdict, in batch clusters seconds apart**, so every row in this database was a preparation record and no fill had ever been recorded. The audit also found the defect underneath the filed one: preparation was not merely stamping fill state, it was *erasing* it, because the upsert wrote every column from the zero value preflight handed it. The fix removes the parameter rather than the habit — `RecordPreparedQuestions` has no summary argument, so preparation cannot claim or erase a fill outcome even if a future call site tries. **Three read-only reviewers then found six more defects in the fix itself, five of them this bug with its sign flipped**, the worst being a marker written before the fill's own expired-posting and bot-check guards, which made the card claim Career Agent had attempted a form on a page that had none. That is the second consecutive task where the review round found more than the implementation did, and the second where a gate box was unchecked by the review of the fix meant to check it. **Eight Pending rows: #551 (1.2, Major), #524 (1.5), #549 (1.0), #552 (1.5), #526 (1.0), #528 (1.0), #553 (0.7), #532 (0.7).** #551 is the open Major and is a `deep-reasoning` row on purpose: `cmd/agent` fills real forms and records nothing, but its browser has closed and its typed values are gone by the time the operator opens the application, so what the card *should* say about work that has evaporated is a product question rather than a missing call. *(Prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 555 | [Known profile values do not reliably prefill employer forms in Assisted Fill](#555-known-profile-values-do-not-reliably-prefill-employer-forms-in-assisted-fill) | Major | Done (2026-08-16) | — | standard | See `documentation/backlog_history/bugs_done_details.md` item #555 for the full account. |
| 556 | [The role gate admits management-track titles ("Director of DevOps") because it only ever checks keyword overlap, never career track vs. seniority](#556-the-role-gate-admits-management-track-titles-director-of-devops-because-it-only-ever-checks-keyword-overlap-never-career-track-vs-seniority) | Major | Done (2026-08-17) | — | standard | See `documentation/backlog_history/bugs_done_details.md` item #556 for the full account. |
...
| 536 | [Tracker email classifier treats promotional emails as `INTERVIEW_REQUESTED`](#536-tracker-email-classifier-treats-promotional-emails-as-interview_requested) | Minor | Done (2026-08-12) | — | standard | See `documentation/backlog_history/bugs_done_details.md` item #536 for the full account. |
...
Fixed 2026-08-14 — full account in `documentation/backlog_history/bugs_done_details.md` item #547.

### 548. Preparing an application stamps a fill outcome, so the card reports a fill that never ran

Closed — full account archived in `documentation/backlog_history/bugs_done_details.md` item #548.

### 551. Automatic Apply fills real employer forms and records nothing, so the card cannot describe its own agent's work

**Found 2026-08-15** by two of the three independent reviewers of bugs.md #548, independently of each
other, while checking whether that fix's invariant held across the whole product.

`FillSummary = what Career Agent actually did to the form` is true of the assisted path after #548.
It is not true of Career Agent as a whole. `cmd/agent/pipeline.go` handles `ErrAwaitingHumanReview` by
logging *"%s form filled — awaiting human review"*, setting `AWAITING_REVIEW`, and calling
`EnsureAssistedPlanForURL` — putting the job straight into the assisted queue. `completedWork` renders
*"Job validated; tailored documents prepared; application form reached and filled for review."* on the
same card. Nothing anywhere writes `assisted_fill_summary`.

Measured: **all 24 queued `AWAITING_REVIEW` rows arrived through that path**, and `execution_logs`
carries an `INITIATED → AWAITING_REVIEW` pair for each of the 11 jobs that also have a summary row —
including 310026, filled 2026-08-12 20:01–20:03 EDT, whose summary row is a preflight record from
2026-08-14 01:33. `application_attempts` does record automatic attempts, but it is keyed by URL, is
never read by `GetFillSummary`, `DeriveFormInventory` or `CompletedSummary`, and holds nothing since
2026-08-12.

**Why this was not folded into #548.** It is not a regression — before that fix these cards said
*"Nothing could be filled automatically"*, which was equally uninformative — and it is not a
mechanical extension of the marker. A copilot-mode fill runs in a browser that then closes, so
whatever it typed is gone by the time the operator opens the application and sees an empty form.
Reporting "8 fields filled" about that would be false in the other direction. What the card should say
about a fill whose work has evaporated is a product decision, which is why this is `deep-reasoning`
rather than a follow-on edit.

### 552. The review clock starts when an application was prepared, so every measurement exceeds the credibility cap and is silently discarded

**Found 2026-08-14** alongside bugs.md #548 and **carried out of it deliberately**, with its evidence,
rather than folded into that fix.

`assistedReviewStartedAt` (`cmd/dashboard/main.go`) reads `assisted_fill_summary.recorded_at` as *"when
this application became ready for the operator to review"*. Preparation writes that column. So on an
application prepared the evening before, the measured review duration is the age of the preparation
run, not the operator's time.

Job 310026: `recorded_at` 2026-08-14 01:33:58, confirmed 2026-08-14 18:57:20 — **17h23m** against
`maxCredibleInteraction` of 30 minutes (`pkg/storage/interactions.go`). `RecordHumanInteraction`
returns `nil` in that case, so the measurement is not merely wrong, it is silently dropped.
`human_interactions` holds **0 rows across the entire database**, which is also why improvements.md
#544 (predicting effort from this installation's own history) has nothing to fit against.

#548 was careful not to feed this clock — `MarkFillAttempted` writes no `recorded_at` on either branch,
and a test pins it — so the defect is exactly as large as it was before that work, and no larger.

### 553. The Assisted card and the Copy Application Packet can describe the same form differently

**Found 2026-08-15** by the inverse-regression reviewer of bugs.md #548.

`AssistedJobCard` renders `CompletedSummary` and, in a panel below it, `CopyPacket`. In one state they
contradict each other: a fill that reached a form on an application which was never preflighted and
which recorded no questions leaves `fill_attempted_at` set and nothing else. The card then says Career
Agent attempted the form, while the packet — whose `DeriveFormInventory` finds no verdict, no questions
and no `filled_count` — says *"Not prepared yet. Career Agent has not read this employer's form."*

The inconsistency is a consequence of a decision that is individually correct on both sides:
`DeriveFormInventory` deliberately does not consult `fill_attempted_at` (ADR-007 decision 8), because
what a fill *did* to a form is a different fact from what Career Agent *knows* about it. Reaching a
form is nonetheless evidence it was read, so the packet is entitled to that signal; the reason it is
not simply wired up is that a `ready` inventory with zero questions and zero fields would then claim
*"Form read. No questions beyond the details above"* about a form nothing ever inventoried, which is
#547's harm rebuilt. The fix needs a state for "a fill saw this form but nothing catalogued it".
...
Fixed 2026-08-16 — full account in `documentation/backlog_history/bugs_done_details.md` item #555.
...
Closed 2026-08-14 — full account archived in `documentation/backlog_history/bugs_done_details.md` item #545.
...
Closed 2026-08-12 — full account archived in `documentation/backlog_history/bugs_done_details.md` item #536.
...
Closed 2026-08-17 — full account archived in `documentation/backlog_history/bugs_done_details.md` item #556.

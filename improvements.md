...
|---|---|---|---|---|---|
| 506 | [`/work_next_item`'s selection rule never returns to `bugs.md` once the gate is MET, starving Minor Pending bugs indefinitely](#506-work_next_items-selection-rule-never-returns-to-bugsmd-once-the-gate-is-met-starving-minor-pending-bugs-indefinitely) | Pending | **2.5** = 5×1.0÷2 | standard | Found 2026-08-01 while selecting this session's item. `bugs.md` currently carries six Minor Pending rows (#480 at 1.67, #501 at 2.0, others lower) that will never again be auto-selected — see detail section |
...
**Found 2026-08-01** while selecting this session's item, re-reading `.agents/prompts/work_next_item.md`'s Step 1 literally.

The selection rule reads: "If it is not yet MET, work the highest-priority open row in `bugs.md`... If the gate is met, or `bugs.md` has no open rows, read the ranked table in `improvements.md`." Once the Usability Gate flips to `MET` (as it did this session, closing #489), every future run goes straight to `improvements.md` and never re-checks `bugs.md` for open rows again — the "gate is met" branch has no path back. This is a one-way ratchet: `bugs.md` currently holds six Minor Pending rows (#480 at 1.67, #501 at 2.0, #490 at 1.5, #482 at 1.33, #504 at 1.5, #503 at 1.5) that scored above several `improvements.md` rows this same session, but none of them will ever be auto-selected again unless a future Major/Blocker bug happens to reopen the gate and a session incidentally fixes them alongside it (as happened with #501-504 riding along on #489's close).

**Proposed direction:** once the gate is `MET`, treat `bugs.md`'s open Minor rows and `improvements.md`'s open rows as one combined pool for selection purposes, picking whichever scores highest by the shared formula — rather than the current all-or-nothing branch. The Blocker/Major severity gate should still control whether `bugs.md` unconditionally outranks everything (unmet case); it should not also suppress every Minor bug once met.

**Acceptance criteria:** re-running `/work_next_item` with the gate MET and at least one `bugs.md` Minor row scoring above every `improvements.md` Pending row picks the bug row, not an improvement row.

**Boundaries:** `.agents/prompts/work_next_item.md` (this project's local prompt file, not the shared library) only — no code change.

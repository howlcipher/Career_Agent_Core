...
**2026-08-02 passive-funnel audit and second groom.** The daemon is active: its latest refresh completed with six eligible rows, which all reached documented terminal outcomes before the queue returned to zero. The audit traced free discovery, queue admission, scoring, document generation, browser submission, persistence, and dashboard telemetry; all additional gaps are already tracked or paid-gated, except #510's missing source-failure/circuit-skip accounting. #510 (1.5) remains the top free mission-facing item, followed by #479 (1.0); all other free Pending rows remain correctly scored. #493, #488, and paywalled #14 remain below-floor user decisions. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` pass.
...
| 510 | [Record discovery-source request failures and circuit-open skips in refresh health](#510-record-discovery-source-request-failures-and-circuit-open-skips-in-refresh-health) | Pending | 1.5 = 6×0.5÷2 | standard | The current Yahoo failure burst is invisible to the persisted per-source error count, leaving an empty queue unexplained. |
| 479 | [A permanent DNS failure spends the full retry/backoff budget instead of failing fast to a terminal status](#479-a-permanent-dns-failure-spends-the-full-retrybackoff-budget-instead-of-failing-fast-to-a-terminal-status) | Pending | 1.0 = 4×0.5÷2 | standard | 3,284 observed retries of one nonexistent hostname show the generic retry budget still wastes real queue capacity. |
...
**Found 2026-08-02** during a live passive-discovery audit. The active daemon's first refresh logged repeated Yahoo `unexpected EOF` transport/read failures, then `Yahoo fallback circuit open; skipping query` messages. The breaker correctly limited the blast radius, and the refresh completed with six new eligible rows, but `DiscoverySourceCounts` only records `AddToFunnel` outcomes. A source request that fails before a result can be parsed therefore contributes neither an error nor a skip to the persisted `discovery_refresh` record. `DiscoverJobs` also treats these per-query failures as non-fatal, so its aggregate `ErrorClass` remains empty.

**Impact:** #509's empty-queue explanation can report the time and number of newly eligible rows, but cannot tell the operator that a discovery source was unavailable or intentionally bypassed. When free structured sources produce no usable postings, this hides a material reason passive discovery under-delivers and makes a network outage indistinguishable from a healthy duplicate-only refresh.

**Fix direction:** add privacy-safe per-source counters for request attempts, final request failures, and circuit-open skips; increment them at the Yahoo fallback boundary (and use the same small interface for other sources where applicable). Persist and expose those aggregates alongside the existing add-to-funnel outcomes. Preserve the current breaker, retry limits, source selection, and all raw-content/privacy boundaries.

**Acceptance criteria:** a deterministic failing Yahoo test records one final failure after retries; a breaker-open test records skipped queries without issuing requests; a successful parsed result retains the existing new/duplicate/excluded accounting; the API/dashboard can distinguish no-results, duplicate-only, source-failed, and source-skipped refreshes without raw URLs or errors.

**Value 6, Effort 2, Decay 0.5, score 1.5.** #509 is a shipped precursor in the same discovery-observability theme, so the score uses one halving; this is a narrow extension that makes its claimed diagnosis truthful during the observed failure mode.
...
**Live evidence 2026-08-02:** `career_agent.log` contains 3,284 instances of the same `lookup wwww.raileurope.com: no such host` resolution failure between 10:50 and 12:16 on 2026-08-01. The subsequent generic-backoff fix prevents the original tight loop, but this is direct evidence that a known-permanent hostname consumes retry budget and queue attention. Value rises from 2 to 4; with the existing 0.5 same-theme decay and Effort 2, the score is now **1.0**.

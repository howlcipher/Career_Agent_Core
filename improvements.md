...
**2026-08-01, session forty-nine.** Improvement #499 is Done: new funnel rows retain their actual discovery channel, so the next nonempty refresh can be analyzed by RemoteOK, Hacker News, ATS feed, SerpApi, or Yahoo rather than destination hostname alone. The dashboard-managed daemon was rebuilt and restarted successfully; its first live aggregate still contains only legacy rows and the eligible queue remains zero, so no channel has been fabricated or claimed for historical data. See `documentation/backlog_history/improvements_done_details.md` item #499 for the full account. Prior status paragraph archived to `documentation/backlog_history/improvements_groom_history.md`.

...
- **Key-Optional Search Fallback**: Always runs the free RemoteOK, Hacker News, and public Greenhouse/Lever feed sources. Role/ATS searches use SerpApi when configured and Yahoo HTML search when no key is present or SerpApi reports an error. The Yahoo path retries a transient failure per query, but a sustained block (bot detection holding a steady failure rate for hours, not a blip) instead trips a source-level circuit breaker: after 5 consecutive final failures it stops sending Yahoo requests for a 2-minute cooldown, then allows one probe through before resuming normally — the same breaker shape used for per-domain job fetch below, applied to discovery instead.
...
- **`duplicate_cooldown_days`**: Optional, disabled at `0`. Set a positive number to skip a job only when a confirmed application within that window has the same normalized company, role family and seniority, location, and remote classification. This is deliberately strict: missing or different location/remote metadata never suppresses a job. Set it back to `0` to override the protection and process all otherwise eligible jobs.
...
`cmd/agent` (and every other command that calls `storage.InitDB`) is the connection that owns the schema and sets WAL mode, via `storage.DSN` in `pkg/storage/dsn.go`; `cmd/dashboard` opens its own read-only connection with `storage.ReaderDSN`, the same helper minus `journal_mode`. That split exists because a read-only connection has no reason to ask SQLite to *change* the journal mode, and doing so can be refused outright (`SQLITE_BUSY`) rather than merely delayed, if another connection already holds an open write transaction on a genuinely fresh database — a five-second busy timeout does not cover that refusal, only an ordinary lock wait. Both DSNs still carry the timeout, so a dashboard query that meets an in-flight write simply waits up to five seconds rather than failing. This matters because `modernc.org/sqlite`, the pure Go driver this project uses, reads pragmas as `_pragma=name(value)` and *silently ignores* the `mattn/go-sqlite3` spelling (`?_journal_mode=WAL`) rather than rejecting it — a connection carrying the wrong form looks configured and is not. You can still start the dashboard before the agent has ever run; it just shows no data until `cmd/agent` creates the schema and sets WAL mode on its first run.

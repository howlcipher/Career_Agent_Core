...
**The gate is still met, and #533 has now been validated against real production mail.** Re-verified 2026-08-07 (evening): zero open Blocker or Major rows. **The tracker was restarted 2026-08-07 15:47 local after a 14-day stop and ran nine consecutive scans with zero errors** — no `Error during scan`, no failed persist, no failed acknowledgement, no warnings, no authentication loop, no database-lock loop; one process, built from `aaebb63` (code-identical to HEAD, the two later commits being `.gitignore`-only), 2.2 CPU-seconds over two hours, sleeping on its normal 15-minute cadence. **#533 works in production:** 120 messages acknowledged and **19 outcome-shaped emails recorded in `unmatched_outcomes`** that the pre-fix code would have acknowledged and destroyed — 12 distinct sender domains, all interview-shaped, max stored field lengths 19 and 27 characters, so no email content leaked into the table. Every #533 invariant was checked against live data rather than assumed: **zero** unmatched rows lack their `processed_emails` acknowledgement (the atomicity guarantee), **zero** message IDs appear twice, and two fully-observed scans re-fetched the same window and wrote nothing at all — idempotency proven without touching the inbox. One ambiguous outcome was exercised and correctly refused to pick an arbitrary application. **#529 was *not* exercised** — no outcome matched any application, so no stage event was written; what was verified instead is that the live database really does carry the `record_funnel_stage_transition` trigger and that it derives `occurred_at`/`reason_code` from exactly the two columns `applyOutcome` now stamps. Unrelated mail modified nothing: **zero** `job_funnel` rows and **zero** stage events were written across all nine scans, and the 7 confirmed applications kept their original timestamps. **The confirmed-application count of 7 was re-verified live, not carried forward.** The validation also found one new defect, filed as **#534**: `StartTracker` only ever fetches the newest ~51 messages, which at this inbox's measured ~12.5 messages/hour is about **4 hours of coverage**, so the 14-day outage put every outcome older than that permanently out of reach — which is why usable outcome data is still **0 matched outcomes**. `go build ./...`, `go vet ./...`, `go test ./...` (including all 22 `pkg/tracker` tests), and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 524 | [Existing queue rows carry no location, so #516's gate cannot screen them](#524-existing-queue-rows-carry-no-location-so-516s-gate-cannot-screen-them) | Minor | Pending — tool shipped, 67/524 backfilled, rest blocked until 2026-08-08 | 1.5 = 3×1.0÷2 | standard | `cmd/backfill-location` shipped and verified live; Greenhouse and Lever are done. Workable holds 361 of the remaining rows and has rate-limited this IP host-wide until ~2026-08-08 03:20 UTC. Re-run `go run ./cmd/backfill-location -confirm` after that — it is re-runnable and skips rows already filled. |
| 534 | [`StartTracker` only ever reads the newest ~51 messages, so any outage longer than the fetch window loses outcomes permanently](#534-starttracker-only-ever-reads-the-newest-51-messages-so-any-outage-longer-than-the-fetch-window-loses-outcomes-permanently) | Minor | Pending | 1.33 = 4×1.0÷3 | standard | Found by the 2026-08-07 production validation, which is also what makes it measurable: the window covers ~4 hours of this inbox, and the tracker had been stopped 14 days. This is the reason usable outcome data is still 0 matched outcomes, and it is the one remaining way a real outcome is lost silently — #533 fixed loss *after* detection, this is loss *before* it. Harmless in steady state, which is why it is Minor. |
...
**Filed 2026-08-07**, by the first production validation of the repaired tracker. This is the last remaining way a genuine outcome email is lost without a trace, and it is the direct reason that validation recorded **0 matched outcomes**.

**The defect.** `pkg/tracker/imap.go:54-62` builds its fetch range as `from = mbox.Messages - 50`, `to = mbox.Messages` — the newest ~51 messages, anchored to the *end* of the mailbox:
...
The code comment already knows this is provisional. Two consequences follow, and the second is the damaging one:

1. A message older than the newest ~51 is never fetched, so it is never classified and never lands in `processed_emails`.
2. Because `from` is derived from `mbox.Messages`, the window only ever moves **forward**. Every new arrival pushes the boundary up. A message that falls out can never come back (short of the user deleting mail, which shrinks `mbox.Messages`). It is not deferred — it is unreachable.

**Why this is not theoretical, measured live 2026-08-07.** After the tracker restarted from a 14-day stop, `processed_emails` gained 51 rows in the first scan and then 2/1/1/13/2/1 in the six that followed — 20 genuinely new messages across 96 minutes, i.e. **~12.5 messages/hour**. Against a 51-message window that is **roughly 4 hours of coverage**. The tracker was stopped from 2026-07-24 to 2026-08-07. Everything that arrived in those 14 days beyond the final ~51 messages was never evaluated and is now permanently out of reach — which is the most plausible explanation for the standing anomaly that **zero** rejection-shaped emails have ever been classified, against 7 confirmed applications and 44 hand-off rows.

**Relationship to #533 — these are different defects, which is why Decay stays 1.0.** #533 fixed loss *after* detection: an outcome was recognised, acknowledged, and discarded. This is loss *before* detection: the message is never fetched, so no code in the tracker ever sees it. #533 shipping did not reduce this defect's cost by any amount, so the same-theme halving does not apply.

**Severity reasoning, written down deliberately.** Filed `Minor`, not `Major`, and not to keep the Usability Gate's zero-Blocker/Major box checked. In steady state the tracker is correct: a 15-minute cadence against ~12.5 messages/hour means ~3 new messages per scan into a 51-slot window, roughly 16× headroom, and the nine scans observed on 2026-08-07 lost nothing. The defect only bites after downtime exceeding the window, and it blocks nobody from using the product today. A future session that disagrees should re-grade it rather than assume the grading was defensive.

**Fix direction.** Do not simply enlarge the constant — that moves the cliff without removing it, and makes every scan re-fetch more mail. Track a durable high-water mark instead and fetch by UID: `UIDVALIDITY` plus the last processed `UID`, persisted, so a scan asks for "everything since I last looked" and downtime costs nothing but a longer catch-up. `go-imap`'s `UidFetch` with a `SeqSet` of `lastUID+1:*` is the shape. Two things must be preserved: the work per scan has to stay bounded so a first run against a large mailbox cannot stall (cap the batch and carry the watermark forward across scans), and `UIDVALIDITY` changing must force a safe rebuild rather than a silent replay — `processed_emails` already makes replay idempotent, verified live, so a rebuild is recoverable but should still be deliberate. An `UNSEEN` search is *not* an adequate substitute: it depends on read state the user mutates by opening mail in any client.

**Regression coverage to add.** A test that a message below the watermark is not re-fetched; a test that a message above it is; a test that a mailbox whose `UIDVALIDITY` changed does not silently skip; and a test that the watermark advances only after the message is durably acknowledged, so a crash mid-scan re-reads rather than skips.

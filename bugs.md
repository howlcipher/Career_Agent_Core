...
| 441 | [A clean setup ends up configured for models the installer never pulled](#441-a-clean-setup-ends-up-configured-for-models-the-installer-never-pulled) | Major | Pending | 7.0 = 7×1.0÷1 | claude-sonnet-4-6 | gemini-3.6-flash-high | — | — | Found while fixing #439, and it is the same defect one layer out: the documented setup is "run `./scripts/install_ollama.sh`, copy `.env.example`", but the installer pulls `llama3.1` + `llava` (`install_ollama.sh:25-26`) while `.env.example` ships **uncommented** `OLLAMA_MODEL="qwen3:30b-instruct"` and `OLLAMA_VISION_MODEL="qwen2.5vl:7b"`. Following both steps therefore configures the agent for two models that were never downloaded, and every text and vision call fails with a model-not-found — exactly #439's failure mode, reached through the README instead of through hardcoded literals |

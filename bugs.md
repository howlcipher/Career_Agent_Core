...
- [x] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **Checked 2026-08-06:** the acceptance trial's last Major closed. #521 (indistinguishable duplicate cards, which caused a false APPLIED record) closed 2026-08-06; #520 (Lever submissions fail in the assisted browser) and #519 (assisted prefill non-functional on Greenhouse/Lever) closed the same day. Only Minor rows (#522-#525) remain open. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 530 | [A posting that has died still occupies a queue card, because the assisted queue never reads the funnel status it selects](#530-a-posting-that-has-died-still-occupies-a-queue-card-because-the-assisted-queue-never-reads-the-funnel-status-it-selects) | Minor | Pending | 1.5 = 3×1.0÷2 | standard | Found while fixing #524, which proved 66 queued postings are gone from their boards and marked them `INVALID_URL` — and none of them left the queue. `GetAssistedQueue` already selects `jf.status` and then discards it. |
...
**Found 2026-08-06** while fixing #524, and confirmed against both the code and the live database rather than inferred from the fix.

#524's backfill asked every queued posting's own board whether it still exists. **66 of them are gone** — an HTTP 404 from the posting's own per-job endpoint, on a board that answered normally for its siblings — and were marked `INVALID_URL` / `expired`. **None of them left the assisted queue.** They are still cards the operator can click, for jobs that cannot be applied to.

**Why.** `GetAssistedQueue` (`pkg/storage/assisted.go:682`) filters on `aa.assisted_state != 'completed'` and nothing else. It *does* select the funnel status — `COALESCE(jf.status, '')` at `:689` — and scans it into a local `currentStatus` at `:715-717` that is then never read again. `serveAssistedQueue` (`cmd/dashboard/main.go:927`) applies no further filter, and it is the only caller. So a row whose posting has died, or which reached any other terminal funnel status, stays in the queue indefinitely and looks identical to a live one.

**Why this is not simply "mark them completed".** `assisted_state = 'completed'` is written in exactly one place — `ConfirmAssistedSubmission` (`:802`), which sets it together with `confirmation_provenance = 'manual_user_confirmation'`. In this schema `completed` means *an application was submitted and the operator confirmed it*. Writing it for a dead posting would fabricate an application record, which is the same class of defect as #518 and #521. #524's backfill deliberately does not do this, and neither should the fix; the queue needs a distinct terminal state, or a status filter, that does not claim an application happened.

**Fix direction.** Either filter `GetAssistedQueue` on the funnel status it already reads (excluding `INVALID_URL` and the other terminal statuses), or give `assisted_applications` a `dismissed`/`unreachable` state separate from `completed`. Prefer whichever keeps `confirmation_provenance` meaning what it means today. Whatever is chosen, the currently-unused `currentStatus` variable is the seam — it is already selected and scanned, so the projection is one condition away from being able to act on it.
...
**Found 2026-08-06** while writing #525's tests, when a straightforward test of `GetAssistedQueue`'s `cover_letter_ready` field failed against a fix that was demonstrably working.

`setupTestDB` (`pkg/storage/manager_test.go:15`) calls `InitDBWithPath(":memory:")`. A `:memory:` SQLite database is **private to one connection**, and `*sql.DB` is a pool. Any query issued while another result set is still open takes a *second* connection from that pool — which opens its own separate, empty in-memory database. The query then fails with `no such table`.

`GetAssistedQueue` (`pkg/storage/assisted.go:658`) does exactly that: inside `for rows.Next()` it calls `assistedDocumentExists` twice per row, each of which runs `conn.QueryRow` for the job's identity. Under test that lookup **always** fails, so `ResumeReady` and `CoverLetterReady` are **always false**, whatever the documents on disk say.

**This is a test-harness artifact, not a production defect — verified, not assumed.** A dashboard built from the current tree and run against the real `applications.db` on `127.0.0.1:8099` reported `resume_ready` and `cover_letter_ready` true for all 524 queued rows. The production database is a file, so every pooled connection opens the same schema.

The cost is entirely in coverage, and it is a coverage gap of the worst kind: the test does not error or skip, it quietly asserts against the failure path. Any real regression in these two fields would pass `go test ./...`. It also silently limits every future test in `pkg/storage` — a nested lookup inside an open iteration cannot be exercised at all, and nothing announces that.

#525's test works around it by driving `assistedDocumentExists` directly and documenting why (`TestAssistedDocumentExists_CoverLetterReadinessFollowsTheMasterLetter`). That covers the helper, not the projection that calls it.

**Fix direction.** Point `setupTestDB` at a per-test file database (`filepath.Join(t.TempDir(), "test.db")`) so the pool's connections share one schema. `file::memory:?cache=shared` is the other option but is process-global, so parallel tests would collide. Then add the `GetAssistedQueue`-level readiness test that cannot be written today, and re-run the full suite: any test that has been passing against the failure path will surface as a change, and each one needs reading rather than mechanically re-baselining.

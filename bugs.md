...
**2026-07-30 live-daemon log audit:** the gate is now **UNMET** because the continuously running service exposed one new Major defect, #466 (retryable queue rows are immediately reselected and can starve the backlog). #467 (browser target closure can abort an individual application attempt) is Minor. These are now the highest-priority bug items ahead of feature improvements.
...
- [x] `go test ./...` passes for every package that has tests — **re-verified 2026-07-30** in both the working tree and a fresh clone, including the 22 tests added for #441. Prior note: **2026-07-28:** the uncached suite is green; focused race suites for the security, scraper, submitter, and agent packages also pass.

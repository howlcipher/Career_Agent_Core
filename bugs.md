...
**MET 2026-08-01 (thirty-ninth session, #489 fix).** #489 (Major) is Done — see its row for the full fix account. `pkg/security/filter.go` now runs a conservative local second pass on top of `promptsec.Moderate()`: it only releases an unsafe result when every decisive threat is a zero-evidence heuristic hit (no located match) in the two categories the audit implicated, the payload carries none of 41 curated injection-marker phrases, and it matches at least 2 of 47 curated ATS-boilerplate signatures — anything with a real located match, a non-heuristic guard hit, or any other threat category still quarantines exactly as before. 79 new test assertions cover both the released false positives (verified against raw `promptsec.Moderate()`, not assumed) and an adversarial camouflaged-injection case that must stay quarantined. **This closes the last open Major/Blocker bug — the zero-Blocker/Major box re-closes**, and all seven gate boxes are now checked. `go build ./...`, `go vet ./...`, `go test ./...` and `gofmt -l ./cmd ./pkg ./internal` all re-run clean. **Caveat, tracked separately rather than blocking this box** (same pattern as the tracker box's #20): #489's own acceptance criteria also called for a live re-measurement showing the Lever/Greenhouse quarantine share actually drop — not retroactively measurable (`job_funnel` never stored posting text for the already-quarantined rows) and filed as **#501** to re-run once fresh data accumulates under the fix. Closing this session also surfaced three more findings from auditing every quarantine call site: **#502** (a third zero-evidence threat source, `encoding.go`'s homoglyph branch, outside #489's fix window by design), **#503** (`TwoStepVerification` never logs its quarantines to the audit CSV, undercounting the true total), and **#504** (our own RAG-generated content can trip the same mechanism via an existing but unverified `QUARANTINED_RAG_CONTEXT` status) — all Minor, none reopening the gate.
...
- [x] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **Re-closed 2026-08-01 (thirty-ninth session):** #489 (Major, the sole open Blocker/Major row) is Done. #501-#504, filed while closing it, are all Minor and do not gate this box. See the gate note above and #489's own row. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 501 | [Re-run #489's aggregate quarantine-rate queries against fresh data once the fix has been live for a batch cycle](#501-re-run-489s-aggregate-quarantine-rate-queries-against-fresh-data-once-the-fix-has-been-live-for-a-batch-cycle) | Minor | Pending | 2.0 = 4×1.0÷2 | mechanical | Found 2026-08-01 closing #489. #489's own acceptance criteria call for a live re-measurement of the Lever/Greenhouse `QUARANTINED_PROMPT_INJECTION` share against the 2026-08-01 baseline (51.0% overall). Not retroactively measurable: `job_funnel`'s schema (`manager.go:68-82`) never stored posting `description`/`rawHTML`, so the 5,983 already-quarantined rows have no original text to replay through the new filter. Value 4: this is the actual confirmation that #489's fix moves the mission's core metric, not just that its unit tests pass. Effort 2: after the daemon has processed a fresh batch under the fix, re-run the same safe aggregate queries #489's audit used (status distribution, hostname breakdown) and compare against the recorded baseline |
...
**Found 2026-08-01** closing #489. #489's own acceptance criteria required a live re-measurement of the Lever/Greenhouse `QUARANTINED_PROMPT_INJECTION` share against the 2026-08-01 baseline (51.0% overall, dominated by `jobs.lever.co`/`greenhouse.io`) to confirm the fix actually moves the mission's core metric, not just that its unit tests pass.

**Why this couldn't be done as part of #489 itself:** `job_funnel`'s schema (`pkg/storage/manager.go:68-82`) has never stored posting `description` or `rawHTML` — only `url`/`company_name`/`job_title`/`status`/timestamps, by design (the quarantine layer's whole point is not retaining attacker-controlled content). The 5,983 rows already marked `QUARANTINED_PROMPT_INJECTION` as of the baseline have no original text left to replay through the new filter, so there is nothing to retroactively re-score.

**What to do:** once the daemon has processed a fresh batch of postings under #489's fix (no special setup needed — this happens automatically as `cmd/agent` runs its normal cycle), re-run the exact aggregate queries #489's audit used against a read-only copy of `applications.db`: status distribution (`QUARANTINED_PROMPT_INJECTION` share of total `job_funnel` rows) and hostname breakdown restricted to rows discovered/processed after the fix shipped. Compare against the recorded baseline (51.0% overall; `jobs.lever.co` and the `greenhouse.io` family dominating the quarantined subset). Also check `prompt_injection_detections.csv` for a drop in the two dominant zero-evidence categories (`instruction_override`, `system_prompt_leak`) without a corresponding rise in `FAILED_SUBMIT`/`BLOCKED_CAPTCHA` (which would suggest jobs are merely failing later instead of being quarantined earlier — a false positive on this fix's success).

**Acceptance criteria:** a materially lower post-fix quarantine share among fresh rows, recorded against the exact baseline numbers in #489's fix account, with no offsetting rise in `FAILED_SUBMIT`/`BLOCKED_CAPTCHA`. If the share does not drop, #489 needs to be reopened with the new evidence rather than this row being closed as "measured, no change needed."

**Safe live verification:** read-only aggregate queries only, same as #489's own audit — never print raw `matched_text`, URLs, company names, or job titles.

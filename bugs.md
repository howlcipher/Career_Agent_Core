...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **re-opened 2026-07-29:** #434 (Major) was filed from a review of the Copilot Mode work. Hand-off statuses (`AWAITING_REVIEW`, `MANUAL_REQUIRED`) are permanent dead ends, so applications the user completes by hand are never recorded and their outcome emails correlate to nothing. Previously MET 2026-07-28 when #112 was resolved; the other three gate boxes remain verified.
...
**Three Pending rows (#434, #435, #433) now hold the gate's zero-open-Major box open again via #434.** Status: **UNMET** pending #434, which is the next autonomous item.
...
|---|---|---|---|---|---|---|---|---|---|
| 434 | [No path moves a job out of `AWAITING_REVIEW` or `MANUAL_REQUIRED`, so hand-off statuses are permanent dead ends](#434-no-path-moves-a-job-out-of-awaiting_review-or-manual_required-so-hand-off-statuses-are-permanent-dead-ends) | Major | Pending | 2.33 = 7×1.0÷3 | claude-sonnet-4-6 | gemini-3.1-pro-high | — | — | Once the user actually submits a hand-off application, nothing can record it. The tracker only updates `APPLIED` rows, so the resulting rejection or interview email matches nothing and the funnel permanently under-reports real outcomes |
| 435 | [`statusReason` is dead code for every status the dashboard actually needs it for](#435-statusreason-is-dead-code-for-every-status-the-dashboard-actually-needs-it-for) | Minor | Pending | 1.5 = 3×1.0÷2 | claude-sonnet-4-6 | gemini-3.6-flash-high | — | — | Found while adding an `AWAITING_REVIEW` case. `statusReason` is only reached from the `SKIPPED` and `FAILED_*` queries, so its `MANUAL_REQUIRED`, `BLOCKED_CAPTCHA`, `INVALID_URL` and new `AWAITING_REVIEW` arms never render. Its unit test passes because it tests the function, not the wiring |
...
**Fix direction:** the queue files are already checklists with `- [ ]` boxes. A small `cmd/` command (or a dashboard action) that reconciles ticked boxes back into `job_funnel` as `APPLIED` would close both gaps and needs no new UI surface. `GetTrackedCompanies` should include both hand-off statuses regardless, so a company is tracked as soon as it is queued.

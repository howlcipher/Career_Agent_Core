...
**UNMET 2026-08-01 (thirty-eighth session, mission-alignment audit).** No bug was fixed this session — a user-directed audit re-verified `RankJobs`, `application_attempts`, the dashboard, and safe aggregate `applications.db`/`applications/prompt_injection_detections.csv` evidence against the project's stated mission (confirmed, submitted, verified applications — not rows processed). It found **#489** (Major, reopening the gate): `promptsec.Moderate()` (bug #394's entire fix, a one-line severity dial from `Strict`) still quarantines roughly half of everything this system ever discovers — `QUARANTINED_PROMPT_INJECTION` is 5,983 of 11,731 `job_funnel` rows (51.0%) lifetime, and disproportionately hits `jobs.lever.co` and the `greenhouse.io` family, the two platforms with dedicated, working auto-submit handlers. 88.5% of the 10,967 logged detections (`instruction_override` and `system_prompt_leak`, the two dominant threat types) carry no located `matched_text` at all — they are fuzzy-keyword-density and coercive-language heuristics, exactly the shape that fires on ordinary EEO/application-instruction boilerplate every real Lever/Greenhouse posting carries. This directly starves the mission's own primary metric: only 2 `APPLIED` rows exist in the entire live database. Also found **#490** (Minor): `job_funnel.applied_at` is declared in the schema but no code path anywhere writes it, so neither of the two real `APPLIED` rows has a timestamp. **#480** (Minor, 1.67) and **#482** (Minor, 1.33) remain open and unchanged. `go build ./...`, `go vet ./...`, `go test ./...` and `gofmt -l ./cmd ./pkg ./internal` all re-run clean — this is a live-behavior/data finding, not a compile or test regression.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **Reopened 2026-08-01 (thirty-eighth session, mission-alignment audit):** **#489** (Major) is Pending — `promptsec.Moderate()` still quarantines roughly half of everything discovered, disproportionately on the two ATS platforms with working auto-submit handlers, and the audit's own database/CSV evidence shows the mission's primary metric (confirmed applications) has been at 2 total the entire time this bug has been live. See the gate note above and #489's own row for the full evidence chain. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
**Mission-alignment audit, 2026-08-01 (thirty-eighth session, backlog-only — nothing implemented).** User-directed audit re-verifying the project against its actual mission (a confirmed, submitted, verified application — not a row processed) using safe read-only aggregate queries against a copy of `applications.db` and the sanitized `applications/prompt_injection_detections.csv` audit log (never the raw job/URL/company content). Filed **#489** (Major, reopens the gate — see the Gate paragraph above for the full evidence) and **#490** (Minor — `job_funnel.applied_at` is schema-declared but never written by any code path, `pkg/storage/manager.go:76` vs. `UpdateFunnelStatus`/`MarkHandoffApplied`, confirmed both live `APPLIED` rows have it `NULL`). Ten new `improvements.md` rows (#491-#500) were also filed from the same audit against the mission brief's candidate list; see that file's own groom note. **#489 is now the outright top of this file by severity** (reopening the gate makes it mandatory regardless of its own formula score) — #480 (1.67) still scores higher and is a reasonable one to work first or alongside it, since a broadened reason-persistence fix is complementary evidence for #489's own root-cause investigation. #482 (1.33) is unchanged. `go build ./...`, `go vet ./...`, `go test ./...` and `gofmt -l ./cmd ./pkg ./internal` all re-run clean; no production code was touched this session.

| # | Bug | Severity | Status | Score (V×D÷E) | Tier | ROI rationale |
|---|---|---|---|---|---|---|
| 489 | [`promptsec.Moderate()` still quarantines roughly half of everything discovered, disproportionately on Lever and Greenhouse](#489-promptsecmoderate-still-quarantines-roughly-half-of-everything-discovered-disproportionately-on-lever-and-greenhouse) | Major | Pending | 1.33 = 8×0.5÷3 | deep-reasoning | Found 2026-08-01, mission-alignment audit. `job_funnel`: 5,983/11,731 rows (51.0%) lifetime are `QUARANTINED_PROMPT_INJECTION`, more than any other status, dominated by `jobs.lever.co` and the `greenhouse.io` family — the two ATS platforms with dedicated auto-submit handlers. `applications/prompt_injection_detections.csv`: 88.5% of 10,967 logged detections have no located `matched_text` (99.3% of `instruction_override`, 100% of `system_prompt_leak`, the two dominant threat types) — pure fuzzy-keyword-density/coercive-language heuristics. Value 8 (ceiling — this is the largest single terminal status in the entire funnel and hits the two platforms most capable of reaching a confirmed application). Decay 0.5: same theme as #394 (Done 2026-07-28), which already shipped once in this exact area — its whole fix was `promptsec.Strict()`→`promptsec.Moderate()` (`a8cfe4a`), a severity dial, not a targeted correction, and the rate is still dominant a week later. Effort 3: build a small benign/malicious regression corpus, determine whether `promptsec` exposes per-guard tuning versus only the global dial, and recalibrate or allowlist the two offending heuristics without weakening real detection |
| 490 | [`job_funnel.applied_at` is declared in the schema but no code path ever writes it](#490-job_funnelapplied_at-is-declared-in-the-schema-but-no-code-path-ever-writes-it) | Minor | Pending | 1.5 = 3×1.0÷2 | mechanical | Found 2026-08-01, same audit as #489. `pkg/storage/manager.go:76` declares `applied_at DATETIME` on `job_funnel` (distinct from `applied_jobs.applied_at`, which IS written). Live-verified: both current `APPLIED` rows have `job_funnel.applied_at IS NULL`. `UpdateFunnelStatus` (`:1159-1179`) and `MarkHandoffApplied` (`:686-726`) — the only two paths that ever set `status='APPLIED'` — never touch the column. Blocks any discovery-to-confirmation duration or confirmed-apps-per-day metric from being computed off `job_funnel` alone (see improvements.md #491). Value 3, Effort 2: set `applied_at = CURRENT_TIMESTAMP`/`time.Now()` at both write sites, plus a migration note that historical `APPLIED` rows will stay `NULL` (no reliable source to backfill from) |
| 482 | [breezy.hr postings are excluded from GetDiscoveredJobs entirely, so they accumulate in DISCOVERED forever with no terminal status](#482-breezyhr-postings-are-excluded-from-getdiscoveredjobs-entirely-so-they-accumulate-in-discovered-forever-with-no-terminal-status) | Minor | Pending | 1.33 = 4×1.0÷3 | standard | Found 2026-08-01 live-verifying #481's fix against the real `applications.db`: all 185 remaining `DISCOVERED` rows are `breezy.hr` postings, and `GetDiscoveredJobs` (`pkg/storage/manager.go:1401`) excludes them via `url NOT LIKE '%breezy.hr%'` (bug #38's fix — 0 `APPLIED` / 48 `FAILED_SUBMIT` for that source). Because the exclusion is in the query itself, these rows never reach scoring, ranking, or the pipeline's expiry/freshness checks — they just sit in DISCOVERED indefinitely, invisible to every dashboard tile and `cmd/requeue` count that filters by status. A smaller, adjacent shape to #481: stuck in DISCOVERED forever via total exclusion rather than being outscored. Value 4: pure hygiene/table-bloat today (185 rows, not yet a performance problem), but the exclusion has no expiry of its own so the count only grows. Effort 3: needs a real terminal status for excluded-source rows at discovery time (or a periodic cleanup job), plus a decision on what status best represents "we chose not to process this source" |
...
| 480 | [UpdateFunnelStatusRetryable never records a status_reason, so every RETRY_EXHAUSTED row loses its own root cause](#480-updatefunnelstatusretryable-never-records-a-status_reason-so-every-retry_exhausted-row-loses-its-own-root-cause) | Minor | Pending | 1.67 = 5×1.0÷3 | standard | Found 2026-08-01, same investigation as #481. All 1,284 `RETRY_EXHAUSTED` rows written 2026-08-01 have `status_reason IS NULL` — `UpdateFunnelStatusRetryable` (`pkg/storage/manager.go:1246-1272`) takes only a URL and never writes `status_reason`, unlike its sibling `UpdateFunnelStatusInvalid(url, reason string)` (`:1198-1207`). All 6 call sites in `cmd/agent/pipeline.go` already have the real error in scope and log it, it just never reaches the database. Value 5: purely diagnostic, but at 1,000+ rows/day this makes throughput problems like #481 much harder to diagnose from the database alone. Effort 3: thread a `reason string` parameter through 6 call sites plus 3 existing test call sites |
...
**Found 2026-08-01**, a user-directed mission-alignment audit re-verifying the project against confirmed, submitted, verified applications rather than rows processed.

**Evidence (job_funnel, safe aggregate query against a read-only copy of `applications.db`):** status distribution across all 11,731 rows: `QUARANTINED_PROMPT_INJECTION` 5,983 (51.0%), `INVALID_URL` 2,943, `RETRY_EXHAUSTED` 1,441, `FAILED_SUBMIT` 523, `BLOCKED_CAPTCHA` 450, `DISCOVERED` 185, `SKIPPED` 156, `MANUAL_REQUIRED` 26, `FAILED_SCORE` 22, `APPLIED` 2. `QUARANTINED_PROMPT_INJECTION` is the single largest terminal status in the table — larger than every successful and every other failed outcome combined. Hostname breakdown of the 2026-08-01 subset (4,658 rows) is dominated by `jobs.lever.co` (1,578), `job-boards.greenhouse.io` (908), `boards.greenhouse.io` (713), `job-boards.eu.greenhouse.io` (327), and `jobs.smartrecruiters.com` (327) — the pre-2026-08-01 subset (1,325 rows) shows the identical pattern (`jobs.lever.co` 732, `boards.greenhouse.io` 139, `jobs.jobvite.com` 121, etc.). **Lever and Greenhouse are the two ATS platforms this project has dedicated, tested, working auto-submit handlers for** (README's Auto-Submit Framework section) — the postings most capable of reaching a confirmed application are the ones being discarded before scoring ever runs.

**Evidence (`applications/prompt_injection_detections.csv`, the sanitized audit log `LogPromptInjectionDetections` writes — improvement #19):** 10,967 logged threat detections. By `threat_type`: `instruction_override` 6,313 (57.6%), `system_prompt_leak` 3,393 (30.9%), `encoding_attack` 1,032, `role_manipulation` 228, `delimiter_injection` 1. By `guard`: `heuristic` 10,266 (93.6%), `sanitizer` 701. **Critically, `matched_text` (the located span that triggered the detection) is empty for 88.5% of all rows — 99.3% of `instruction_override` and 100.0% of `system_prompt_leak`, the two dominant categories.** The library's own `message` field for these is generic and density/heuristic-based, not a literal match: `"fuzzy match detected multiple injection-related keywords (possible typo evasion)"` (6,002 of 6,313 `instruction_override` rows) and `"coercive attempt to extract sensitive data"` (3,012 of 3,393 `system_prompt_leak` rows) — no code bug in this repo's own conversion (`toStoredThreats`, `pkg/submitter/browser.go:37-51`, copies `t.Match` directly); the third-party `promptsec` heuristic guard itself returns these two categories without a locatable span by design. This is exactly the shape of heuristic that fires on ordinary corporate ATS boilerplate: EEO/demographic voluntary-disclosure sections (which legitimately ask for "sensitive data" from the *candidate*) and standard multi-step application instructions, both universal on Lever and Greenhouse postings specifically.

**Root cause of the fix gap:** bug #394 ("QUARANTINED_PROMPT_INJECTION has massive false positive rate on legitimate jobs," Done 2026-07-28) shipped exactly one change (`a8cfe4a`): `pkg/security/filter.go`'s `NewQuarantineLayer` switched `promptsec.Strict()` → `promptsec.Moderate()`. That is a global severity dial, not a targeted correction of the two offending heuristics, and — per the evidence above — it did not resolve the underlying problem; `QUARANTINED_PROMPT_INJECTION` has remained the dominant terminal status both before and during this audit.

**Mission impact:** the project's own stated primary metric is a confirmed, submitted, verified application. The live database has exactly 2 `APPLIED` rows in its entire history (both 2026-07-29, `boards.eu.greenhouse.io`). A defect discarding roughly half of everything the system ever discovers — concentrated on the two platforms most able to reach a confirmed application — is a more direct block on that metric than any queue-ranking or scheduling issue (#480, #481, #482) in this file.

**Fix direction (not yet designed — this audit's scope is filing, not implementing):** (1) build a small regression corpus of real, anonymized-but-representative benign ATS boilerplate (EEO statements, multi-step application instructions, accessibility text) that must NOT be quarantined, plus a set of genuine prompt-injection/exfiltration payloads that MUST still be caught; (2) check whether `promptsec` exposes per-guard enable/disable or threshold tuning below the `Strict`/`Moderate`/`Lenient` global dial for the `instruction_override`/`system_prompt_leak` heuristic guards specifically, since those two account for 88.5% of the zero-evidence detections; (3) if no finer control exists upstream, evaluate a local allowlist/second-pass check for known-benign ATS boilerplate patterns before trusting a zero-evidence heuristic hit, without weakening detection of anything with a real located match or from the `sanitizer` guard. Cross-references: #480 (broadening `status_reason` persistence across all `UpdateFunnelStatus*` writers, not just `UpdateFunnelStatusRetryable`, would let `job_funnel` itself carry the triggering `message` instead of requiring a join to the CSV); improvements.md #494 (append-only stage ledger) would give this investigation per-attempt visibility the current schema cannot.

**Acceptance criteria:** the regression corpus's malicious payloads are still quarantined 100% of the time; the corpus's benign-boilerplate payloads are no longer quarantined; a live re-measurement of the Lever/Greenhouse `QUARANTINED_PROMPT_INJECTION` share (using the same hostname-aggregate method as this row's evidence) drops materially from the 2026-08-01 baseline (51.0% overall, ~34% of Lever traffic, ~63%+ combined of Greenhouse-family traffic — recompute precisely at fix time) without a corresponding rise in `FAILED_SUBMIT`/`BLOCKED_CAPTCHA` that would suggest jobs are merely failing later instead of being quarantined earlier.

**Automated tests:** table-driven tests feeding the regression corpus through `QuarantineLayer.CheckPayload`/`CheckPayloadDetailed` asserting `Safe`/`!Safe` matches the corpus's own labels; a test asserting the specific benign phrasings found in this audit's `message` samples no longer trigger.

**Safe live verification:** re-run this row's own aggregate queries (status distribution, hostname breakdown, `matched_text` emptiness by `threat_type`) against a fresh read-only copy of `applications.db` and the current `prompt_injection_detections.csv` after the fix ships, comparing against the exact baseline numbers recorded above. Never print raw `matched_text`, URLs, company names, or job titles.

**Boundaries:** do not disable or bypass the quarantine layer wholesale — it is the project's own documented security boundary (README's Security Quarantine feature, ADR-002-Prompt-Injection.md) and a genuine prompt-injection attempt must still be caught. Do not weaken the `sanitizer` guard or any detection with a real located match; this row's evidence only implicates the two zero-evidence heuristic categories.

### 490. `job_funnel.applied_at` is declared in the schema but no code path ever writes it

**Found 2026-08-01**, same audit as #489. `pkg/storage/manager.go:76` declares `applied_at DATETIME` as a column on `job_funnel` — distinct from `applied_jobs.applied_at` (`:54`), a different table, which IS written by `RecordApplicationInDB` (`:646`) and `MarkHandoffApplied` (`:717`).

**Live-verified:** a safe read-only query against a copy of `applications.db` confirmed both of the database's two `APPLIED` `job_funnel` rows have `applied_at IS NULL`.

**Root cause:** the only two code paths that ever set `job_funnel.status = 'APPLIED'` are `UpdateFunnelStatus(url, status)` (`:1159-1179`, called from `cmd/agent/pipeline.go:598` on a real auto-submit) and `MarkHandoffApplied` (`:686-726`, called from `cmd/reconcile` when the user ticks a hand-off checklist box). Neither statement includes `applied_at` in its `SET` clause — only `status` and `last_updated`.

**Impact:** `job_funnel.applied_at` cannot answer "when was this confirmed applied," "how long from discovery to confirmation," or "how many confirmed applications per day" on its own — exactly the metrics improvements.md #491 (mission metrics) needs. A workaround (joining to `applied_jobs.applied_at`) exists but is unreliable on its own: this audit's aggregate check found 22 `applied_jobs` rows with no matching `job_funnel` row at all, and several more whose `job_funnel.status` has since moved away from `APPLIED` (both fully explained by legacy pre-#94/#412 dedup rows for non-posting `workday.com` subdomains, not a live defect — but not a clean source to build a mission metric on either).

**Fix direction:** add `applied_at = ?` (UTC, matching `last_updated`'s existing convention and its documented lesson about mixed local/UTC formats — see the comment at `manager.go:1164-1171`) to both `UPDATE` statements, set to the same timestamp already used for `last_updated`. Historical `APPLIED` rows (both of them, today) stay `NULL` — there is no reliable source to backfill from, and this row does not attempt one.

**Acceptance criteria:** a fresh `UpdateFunnelStatus(url, "APPLIED")` or `MarkHandoffApplied` call populates `applied_at` with a real, parseable UTC timestamp; every other status transition leaves it untouched (never spuriously overwritten if a row is later reprocessed).

**Automated tests:** a storage-package test asserting `applied_at` is set (non-null, UTC, parses) after each of the two write paths, and unchanged after every other `UpdateFunnelStatus*` call.

**Safe live verification:** after the fix ships and the next real `APPLIED` transition occurs, a read-only aggregate query confirming that row's `applied_at` is populated and its value is later than `discovered_at`.

**Boundaries:** no backfill of the two existing historical `APPLIED` rows; no change to `applied_jobs.applied_at`, which already works correctly.
...
**Root cause:** `storage.GetDiscoveredJobs` (`pkg/storage/manager.go:1401`) filters `AND url NOT LIKE '%breezy.hr%'` — bug #38's fix, excluding a source that measured 0 `APPLIED` against 48 `FAILED_SUBMIT`. That exclusion is correct as far as it goes, but it happens entirely inside the query that feeds the pipeline: a `breezy.hr` row is never loaded, never scored, never ranked, and never reaches any of the terminal-status paths (`UpdateFunnelStatusInvalid`, `UpdateFunnelStatusRetryable`, a successful `APPLIED`) that every other `DISCOVERED` row eventually hits one way or another. It just sits at `DISCOVERED` permanently, with no expiry check ever touching it (`checkJobAlive` and friends only run once a job reaches the pipeline).

**Impact today:** small — 185 rows is noise against the table's other statuses (5,982 `QUARANTINED_PROMPT_INJECTION`, 2,935 `INVALID_URL`, etc.), and these rows are invisible to every dashboard tile and `cmd/requeue -stats` count that groups by status, so they cost nothing functionally. The risk is unbounded growth with no expiry: as discovery keeps finding new `breezy.hr` postings (assuming that source is still crawled upstream of this exclusion), the count only grows, forever, with no path back to a terminal state. Worth checking whether discovery itself should stop inserting `breezy.hr` rows in the first place, which would be a smaller fix than adding a new terminal status.

**Fix direction:** either (a) have discovery skip inserting `breezy.hr` rows at all, if that's where they enter `job_funnel`, or (b) give `GetDiscoveredJobs`'s exclusion a corresponding write path — e.g. a periodic sweep that marks already-excluded-source `DISCOVERED` rows as `SKIPPED` (a status that already exists in the table) instead of leaving them silently invisible. Not urgent at 185 rows; worth a look before it becomes thousands.
...
**Found 2026-08-01**, same investigation as #481. All 1,284 `RETRY_EXHAUSTED` rows written on 2026-08-01 have `status_reason IS NULL`:
```
SELECT status_reason, COUNT(*) FROM job_funnel WHERE status='RETRY_EXHAUSTED' AND substr(last_updated,1,10)='2026-08-01' GROUP BY status_reason;
(None, 1284)
```
Every row, no exceptions — not a sampling artifact.

**Root cause:** `storage.UpdateFunnelStatusRetryable(url string) error` (`pkg/storage/manager.go:1246-1272`) takes only a URL and writes:
```go
"UPDATE job_funnel SET status = 'RETRY_EXHAUSTED', retry_count = ?, last_updated = ? WHERE url = ?"
```
— no `status_reason` column in the statement at all. Its sibling `UpdateFunnelStatusInvalid(url, reason string) error` (`:1198-1207`), used for the `INVALID_URL` terminal status, already takes and persists a `reason` argument, which is exactly why `INVALID_URL` rows are queryable by `expired`/`malformed` today and `RETRY_EXHAUSTED` rows are not. All 6 call sites in `cmd/agent/pipeline.go` (`:109`, `:144`, `:215`, `:289`, `:303`, `:444`) already have a real, specific error in scope at the call (`err`/`checkErr`) and log it via `log.Printf` one line above the call — the information exists and is even printed, it just never reaches the database.

**Impact:** a `RETRY_EXHAUSTED` row (1,439 total historical rows, 1,284 in a single day this session) carries no queryable trail of *why* it exhausted its retry budget — network error, Ollama timeout, a specific HTTP status, a Playwright failure — forcing anyone diagnosing a throughput problem (like #481, filed alongside this) back to grepping `career_agent.log` by timestamp instead of querying the database the dashboard and `cmd/requeue` already use as their source of truth. The dashboard's `RETRY_EXHAUSTED` card (improvements.md #468) shows a count with no way to break it down by cause.

**Value 5:** purely diagnostic — nothing crashes or misbehaves — but the funnel's entire retry/backoff machinery (bug #466) becomes much harder to reason about at the exact scale (1,000+ rows/day) where reasoning about it matters most. **Effort 3**: `UpdateFunnelStatusRetryable` needs a `reason string` parameter threaded through all 6 call sites, each of which already has the error value in scope, plus updating the existing test call sites in `pkg/storage/manager_test.go` (`:658`, `:1247`, `:2149`). Score = 5×1.0÷3 = **1.67**.

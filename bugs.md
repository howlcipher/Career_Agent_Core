...
- [x] `go build ./...` succeeds clean — re-verified 2026-07-26
- [x] `go vet ./...` reports no issues — re-verified 2026-07-26
- [ ] `go test ./...` passes for every package that has tests — **regressed 2026-07-26:** six `pkg/submitter` tests fail against the uncommitted resume-upload work tracked as #118. Build and vet remain clean; the existing worktree change is preserved, not overwritten by this audit.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **regressed 2026-07-26:** the application sweep filed #118-#129 and reopened the unresolved half of #112. The earlier 2026-07-24 evidence remains valid historically, but the gate is now open again.
...
**Status as of 2026-07-26 application sweep: UNMET.** Build and vet are clean, but the test suite is red under in-progress #118 and the sweep found open Blocker/Major defects. Bug work again outranks `improvements.md`.
...
**2026-07-26 application-sweep note:** re-ran the gate, reviewed the current journal, traced the agent, discovery, submission, tracker, storage, dashboard and security boundaries, and inspected permissions without reading or printing protected file contents. `go build ./...` and `go vet ./...` are clean. `go test ./...` currently has six failures, all in `pkg/submitter`, against the user's existing uncommitted #118 work. The model columns below use models confirmed available on this machine today: `claude-opus-4-6-thinking` / `claude-sonnet-4-6` and `gemini-3.1-pro-high` / `gemini-3.6-flash-high`. The available `gpt-oss-120b-medium` and local `qwen3:30b-instruct` remain valid bounded-task alternatives, but this backlog's established schema has only Claude and Gemini recommendation columns.

| # | Bug | Severity | Status | Score (V×D÷E) | Claude model | Gemini model | ROI rationale |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 118 | [Resume-selector fallback work breaks every submitter path without a readable resume](#118-resume-selector-fallback-work-breaks-every-submitter-path-without-a-readable-resume) | Major | Pending — implementation in worktree, tests red | 7.0 = 7×1.0÷1 | claude-sonnet-4-6 | gemini-3.6-flash-high | Highest priority because it makes the repository's required verification loop fail. The new fallback is directionally correct and preserves the live Pinpoint finding, but `attachResume` reads `resumePath` before establishing whether the current mapping even has a resume control. Six established orchestration tests now fail before reaching what they were written to test |
| 119 | [Free discovery sources are disabled when the SerpApi key is absent](#119-free-discovery-sources-are-disabled-when-the-serpapi-key-is-absent) | Major | Pending | 7.0 = 7×1.0÷1 | claude-sonnet-4-6 | gemini-3.6-flash-high | `DiscoverJobs` returns on a missing `SERPAPI_API_KEY` before RemoteOK, Hacker News, and public Greenhouse/Lever feeds run. That makes the shipped free sources unreachable in the no-key configuration this project explicitly supports |
| 124 | [The email tracker acknowledges a message even when its database update fails](#124-the-email-tracker-acknowledges-a-message-even-when-its-database-update-fails) | Major | Pending | 6.0 = 6×1.0÷1 | claude-sonnet-4-6 | gemini-3.6-flash-high | `updateDBWithTrackerResult` discards `db.Exec`'s error and the caller then records the IMAP Message-ID as processed. A lock, disk, or schema failure can permanently lose the only outcome signal while the logs claim the DB was updated |
| 126 | [The unauthenticated dashboard binds every network interface while announcing localhost](#126-the-unauthenticated-dashboard-binds-every-network-interface-while-announcing-localhost) | Major | Pending | 6.0 = 6×1.0÷1 | claude-sonnet-4-6 | gemini-3.6-flash-high | `cmd/dashboard` prints `localhost:8080` but listens on `:8080`, exposing application counts, company/title context and posting URLs to the LAN with no authentication. A loopback default plus server timeouts is a small fix with immediate privacy value |
| 127 | [Sensitive credentials application data and generated documents are world-readable](#127-sensitive-credentials-application-data-and-generated-documents-are-world-readable) | Major | Pending | 3.5 = 7×1.0÷2 | claude-sonnet-4-6 | gemini-3.6-flash-high | Verified from metadata only: `.env`, `pii.yaml`, SQLite files, source resumes/letters and generated application artifacts are mode `0644`; generated directories are `0755`. Gitignore prevents commits, not access by another local account. Storage also hard-codes those permissive modes |
| 123 | [Failed and non-2xx job-page fetches still proceed to expensive fit scoring](#123-failed-and-non-2xx-job-page-fetches-still-proceed-to-expensive-fit-scoring) | Major | Pending | 3.0 = 6×1.0÷2 | claude-sonnet-4-6 | gemini-3.6-flash-high | A transport error leaves the description empty and continues; HTTP 404/500 bodies are accepted as job text; response closes are deferred until the worker exits. This can spend the slowest model call on a title or error page and make a submission decision without a valid posting |
| 129 | [The agent hard-codes one developer-specific career-profile path](#129-the-agent-hard-codes-one-developer-specific-career-profile-path) | Major | Pending | 3.0 = 6×1.0÷2 | claude-sonnet-4-6 | gemini-3.6-flash-high | `cmd/agent` always reads `/var/home/howlcipher/dev/ai_knowledge_library/USER_PROFILE.md`; `cmd/reingest` repeats it as its default. On any other checkout the RAG cache silently stays empty or stale, degrading grounded scoring and screening answers |
| 121 | [Untrusted job text reaches embedding and scoring models before quarantine](#121-untrusted-job-text-reaches-embedding-and-scoring-models-before-quarantine) | Blocker | Pending | 2.67 = 8×1.0÷3 | claude-opus-4-6-thinking | gemini-3.1-pro-high | The main worker embeds and scores fetched job content before any security scan. Its only pre-score `CheckPayload` scans trusted retrieved résumé context instead. A later browser-path scan cannot protect scoring or dedicated ATS paths from prompt injection |
| 120 | [`--daemon` logs a six-hour drip mode but exits after one batch](#120---daemon-logs-a-six-hour-drip-mode-but-exits-after-one-batch) | Major | Pending | 2.33 = 7×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | The flag changes one log line only. There is no outer cycle, batch cap, cancellable wait, or queue refresh; `wg.Wait()` is followed by “Batch execution complete” and process exit. The README and improvement #10 both promise behavior that does not exist |
| 122 | [SSRF defenses block literal private IPs but not hostnames that resolve to them](#122-ssrf-defenses-block-literal-private-ips-but-not-hostnames-that-resolve-to-them) | Blocker | Pending | 2.0 = 8×1.0÷4 | claude-opus-4-6-thinking | gemini-3.1-pro-high | HTTP, redirect, Hacker News and Playwright checks compare literal hostnames or call `net.ParseIP` without DNS resolution. A public-looking name resolving or rebinding to loopback/private/link-local space bypasses the documented boundary |
| 128 | [Saving a second role at the same company overwrites the first role's documents](#128-saving-a-second-role-at-the-same-company-overwrites-the-first-roles-documents) | Major | Pending | 2.0 = 6×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | `SaveApplication` keys the document directory only by sanitized company name and rewrites fixed filenames. Multiple roles—or sanitized-name collisions—replace earlier resume, cover letter, prep and metadata, and concurrent workers can race |
| 125 | [One outcome email updates every active application at the matched company](#125-one-outcome-email-updates-every-active-application-at-the-matched-company) | Major | Pending | 1.75 = 7×1.0÷4 | claude-opus-4-6-thinking | gemini-3.1-pro-high | Tracker correlation stops at company name, then updates every `APPLIED` row for that company. With two roles open, one rejection or interview changes both; ambiguous messages should never be bulk-applied |
| 112 | [The same posting exists twice, once per URL scheme, and their statuses have diverged](#112-the-same-posting-exists-twice-once-per-url-scheme-and-their-statuses-have-diverged) | Major | Pending — dedup fixed; funnel merge unresolved | 1.5 = 6×1.0÷4 | claude-opus-4-6-thinking | gemini-3.1-pro-high | Measured: **20 scheme-duplicate pairs** in `job_funnel`, **11 holding different statuses**. Dedup now prevents a second outward application, but discovery, queueing and reporting still operate on two independently mutable rows. Reopened by the 2026-07-26 journal/backlog sweep instead of leaving unresolved work labelled Resolved |
...
### 118. Resume-selector fallback work breaks every submitter path without a readable resume
...
**Acceptance criteria:** keep the mapped-selector-plus-fallback behavior; do not read a file when no resume control is present or required; fail clearly when a required resume exists but its file is unreadable; preserve optional-resume behavior; add focused tests for mapped, fallback, absent and unreadable cases; restore `go test ./...` without weakening the six existing tests. This audit does not edit the user's in-progress implementation.

### 119. Free discovery sources are disabled when the SerpApi key is absent
...
### 125. One outcome email updates every active application at the matched company

The tracker matches an email to a company using sender-domain/subject heuristics, then executes:

`UPDATE job_funnel SET status = ? WHERE company_name = ? AND status = 'APPLIED'`

There is no role, posting URL, requisition ID or thread correlation, so every open application at that company receives the same outcome. Exact company matching prevents unrelated-company corruption but does not solve multiple roles at one employer.

**Acceptance criteria:** correlate to one application using stable ATS/requisition metadata and normalized role clues; if more than one candidate remains, record an explicit unmatched/ambiguous event for manual review and update none; never bulk-update on company alone; add two-role tests for rejection, interview and ambiguous subjects.
...
### 112. The same posting exists twice, once per URL scheme, and their statuses have diverged (Pending — dedup fixed; funnel merge unresolved)

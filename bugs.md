...
- [x] A working local Ollama install with the models `cmd/agent` needs — re-verified 2026-07-27 through the live tags endpoint: `qwen3:30b-instruct`, `qwen2.5vl:7b`, and `nomic-embed-text` are present, along with `qwen3:4b-instruct`. Note: this box's original wording (`llama3.1`, `llava`, `nomic-embed-text`) is stale; `.env.example` carries the current recommendations.
...
- [x] `cmd/dashboard` serves and displays live, correct data from a populated `applications.db` — re-verified 2026-07-27 without printing response data: the already-running root page and `/api/metrics` both returned HTTP 200. The listener is currently `*:8080`, which independently reconfirms bug #126. The original 2026-07-20 verification compared live counts with the database.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **9 remain after #124 was resolved and #125's remaining fail-closed correlation gap was reclassified Minor on 2026-07-27.** The application sweep filed #118-#129 and reopened the unresolved half of #112. The earlier 2026-07-24 evidence remains valid historically, but the gate is now open again.
...
**Status after the 2026-07-27 groom: UNMET.** Build, vet and the full test suite are clean, but 9 open Blocker/Major defects remain. Bug work continues to outrank `improvements.md`.
...
| 125 | [Ambiguous outcome emails retry forever instead of entering manual review](#125-ambiguous-outcome-emails-retry-forever-instead-of-entering-manual-review) | Minor | Pending — bulk update mitigated; manual correlation unresolved | 0.75 = 3×1.0÷4 | claude-sonnet-4-6 | gemini-3.6-flash-high | #124 now rolls back a company match affecting multiple `APPLIED` rows and leaves the message unacknowledged, preventing the original bulk corruption. The remaining gap is that the same ambiguous email fails every poll with no durable manual-review event or role-level correlation |

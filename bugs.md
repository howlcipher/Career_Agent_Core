...
**The gate is now unmet because the trial exposed three open Major defects (#519, #520, #521) that were filed rather than fixed, at the operator's instruction.** The most consequential is #519: Assisted Apply cannot prefill on Greenhouse or Lever, so all five applications were typed by hand despite the pipeline having already filled each form successfully — the feature's core promise is unmet on every application. #521 is a data-integrity defect that already produced one false `APPLIED` record. #520 leaves roughly half the ATS market unusable through the assisted browser. Four Minor rows (#522-#525) were filed alongside them. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **Unchecked 2026-08-06:** three Major rows are open from the acceptance trial (#519 assisted prefill non-functional on Greenhouse/Lever, #520 Lever submissions fail in the assisted browser, #521 indistinguishable duplicate cards caused a false APPLIED record). All were filed deliberately rather than fixed, at the operator's instruction, to be scheduled as their own tasks. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
| 519 | [Assisted Apply cannot prefill on Greenhouse or Lever, the only two ATSes it is used with](#519-assisted-apply-cannot-prefill-on-greenhouse-or-lever-the-only-two-atses-it-is-used-with) | Major | Pending | 2.5 = 5×1.0÷2 | deep-reasoning | This is the feature's entire promise. All five applications in the acceptance trial were typed by hand despite the pipeline having already filled each form successfully. |
...
| 520 | [Lever submissions fail inside the assisted browser but succeed in an ordinary one](#520-lever-submissions-fail-inside-the-assisted-browser-but-succeed-in-an-ordinary-one) | Major | Pending | 1.6 = 4×1.0÷2.5 | deep-reasoning | Lever is roughly half the ATS market and is currently unusable through the assisted browser. Cause is unknown, so effort is uncertain — the reproduction is clean but the fix is not yet scoped. |
...
**Found 2026-08-06**, raised by the operator after all five trial applications had to be typed by hand.

**Mechanism.** `AttemptSubmit` routes by domain: Greenhouse URLs go to `handleGreenhouse` (`pkg/submitter/browser.go:1631`) and Lever URLs to `handleLever` (`:1648`). These dedicated handlers fill the form successfully — the trial logged `form filled — awaiting human review` for all six jobs processed — but they **never call `SaveFormMapping`**. Only the Learner Module path does that (`:1723`). After four successful Greenhouse fills, `form_mappings` still held 80 rows and **zero** for Greenhouse or Lever.

Assisted Apply's refill, `FillAssistedMappedPage` (`:4176`), only knows how to replay a cached dynamic mapping:

```go
mapping, err := storage.GetFormMapping(domain)
if err != nil { return fmt.Errorf("no reusable form mapping: %w", err) }
```

So on precisely the two platforms the feature is used with, Continue can never refill and logs "refill stopped safely" instead. Compounding it, the pipeline's fill happens in a throwaway headless browser whose state dies with the process; the assisted browser opens a fresh, blank session.

Net effect: the agent fills each form, discards the work, and the operator retypes it. That is the feature's entire promise, unmet on every application.

**Fix direction.** Give `FillAssistedMappedPage` the same ATS routing the automatic path already has — `handleGreenhouse` / `handleLever` / `handleAshby` when the domain matches, falling back to the cached mapping otherwise. The handlers already accept a `fillTarget` and already honour `copilotMode`.

**Do not ship without verifying this:** those handlers were written for the automatic path, where they also click Submit. Confirm on a real application that `copilotMode: true` stops them before submission in the assisted browser. A regression here would click Submit on the operator's behalf, which is the one thing assisted mode must never do.
...
**Found 2026-08-06** during the acceptance trial, and only correctly diagnosed on the fifth application.

**Evidence.** Inside the assisted browser: Greenhouse succeeded 4 of 4 (Grafana Labs, Affirm, Smartsheet, Temporal); Lever failed every time with *"There was an error verifying your application. Please try again."* The same Veeva posting that failed in the assisted browser then **submitted successfully in the operator's own Chrome**. So the variable is the assisted browser combined with Lever specifically.

Three wrong conclusions were reached before this one, each retracted on evidence: that the `NetworkGuard` proxy or route guard was blocking CAPTCHA infrastructure (disproven — every reCAPTCHA/hCaptcha/Turnstile host resolves and is allowed in 7-27ms, and Playwright never routes `data:`/`blob:` requests at all); that jobgether was uniquely broken (partly true, and it is separately excluded, but Veeva failed too); and that Lever was broken platform-wide for this environment (disproven by the successful own-browser submission).

**Unknowns.** Not yet investigated: whether the failure is the authenticated `HTTPProxy`, the `AddInitScript` fingerprint spoofing (`navigator.webdriver`, a `plugins` array of `[1,2,3,4,5]`, which is itself a detection tell), `--no-sandbox`, or the CDP debugging pipe. Effort is scored pessimistically because the reproduction is clean but the cause is not scoped.

**Constraint on any fix.** Do not pursue this by making the browser harder to detect. If a site's verification legitimately refuses an automated browser, the correct answer is to hand the operator a direct link to open themselves — which is exactly how the trial's fifth application was completed.

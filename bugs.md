...
**The gate remains unmet on two open Major defects, down from three: #519 closed 2026-08-06.** Assisted Apply now prefills through the same dedicated Greenhouse/Lever/Ashby handlers the automatic path uses, verified live against a real Greenhouse posting and a real Lever posting — fields, location, country, résumé, and cover letter landed on the form, the URL never changed, and nothing was submitted. That live run also found and fixed a second defect affecting the automatic path too: the modern Greenhouse board names its upload control `id="resume"` with no `name`, so the résumé had been silently going unattached. Still open: #521, a data-integrity defect that already produced one false `APPLIED` record, and #520, which leaves Lever submissions failing inside the assisted browser (#519 makes that form arrive prefilled but does not change whether Lever accepts it). Four Minor rows (#522-#525) remain. `go build ./...`, `go vet ./...`, `go test ./...`, and `gofmt -l ./cmd ./pkg ./internal` all pass. *(prior status paragraph archived in `documentation/backlog_history/bugs_groom_history.md`.)*
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **Unchecked 2026-08-06:** two Major rows remain open from the acceptance trial (#520 Lever submissions fail in the assisted browser, #521 indistinguishable duplicate cards caused a false APPLIED record). #519 (assisted prefill non-functional on Greenhouse/Lever) closed 2026-08-06. *(prior verification history archived in `documentation/backlog_history/bugs_groom_history.md`.)*

...
| 130 | [Yahoo fallback drops discovery on transient unexpected EOF responses](#130-yahoo-fallback-drops-discovery-on-transient-unexpected-eof-responses) | Major | Pending | 2.0 = 6×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | `gpt-5.6-terra` | Network retry/backoff behavior needs balanced implementation and verification. | The live log recorded 148 Yahoo request failures with `unexpected EOF` in one continuous run. `discoverWithYahooHTML` returns immediately on the transport error, so each failed role/ATS query loses its discovery opportunity and receives no bounded retry or source-health signal |
| 131 | [ATS board polling discards truncated JSON without retry](#131-ats-board-polling-discards-truncated-json-without-retry) | Minor | Pending | 1.67 = 5×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | `gpt-5.6-terra` | Parser and polling resilience is a moderate, testable reliability change. | Seven `unexpected end of JSON input` responses affected four known Greenhouse/Lever boards. `pollBoard` logs the parse error and skips the entire board, so a transient or rate-limited partial response silently loses all current postings from that company for the pass |

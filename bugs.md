...
| 439 | [The NLP microservice call ignores your LLM configuration and hardcodes a model this host does not have](#439-the-nlp-microservice-call-ignores-your-llm-configuration-and-hardcodes-a-model-this-host-does-not-have) | Major | Pending | 4.0 = 8×1.0÷2 | claude-sonnet-4-6 | gemini-3.1-pro-high | — | — | Found while documenting `nlp_service` for the README pass. `ProcessJobApplication` hardcodes `"provider": "ollama"` and `"model": "llama3"` into the request payload, so with `use_master_cover_letter: false` the entire tailored-document path ignores `LLM_PROVIDER` and `OLLAMA_MODEL`. Verified live against `localhost:11434/api/tags`: `llama3` **is not installed** — the installed models are `qwen3:30b-instruct`, `qwen3:4b-instruct`, `qwen2.5vl:7b` and `nomic-embed-text`. So the flagship tailoring feature cannot currently succeed at all on this host |

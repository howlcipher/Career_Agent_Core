...
**Found 2026-08-01** while completing #501 with safe, sanitized aggregate queries. The daemon binary was rebuilt and started at 19:21 EDT, after #489's 18:51 EDT fix commit. Of the first eight rows it processed after that commit, none were `QUARANTINED_PROMPT_INJECTION` (0.0%, down from the 51.0% lifetime baseline), none were `BLOCKED_CAPTCHA`, and all eight were `FAILED_SUBMIT`. The lifetime `FAILED_SUBMIT` share is 531/11,739 (4.5%), so this is an outcome-offsetting result, not evidence that #489 has increased confirmed applications.

**Fix direction:** aggregate only sanitized error/status-reason categories from the post-fix cohort, trace the dominant category to the pipeline branch, and add a regression test for the identified terminal path. Do not print or persist raw URLs, company names, job titles, posting content, or audit-log matched text.

**Acceptance criteria:** a post-fix cohort large enough to distinguish a dominant failure category, a tested fix if one is found, and a follow-up aggregate showing the lower quarantine rate is not simply exchanged for a comparable `FAILED_SUBMIT` or `BLOCKED_CAPTCHA` rate.

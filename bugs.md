...
| 531 | [Every queue card whose timestamp was written by Go reports `last_updated` as year 0001](#531-every-queue-card-whose-timestamp-was-written-by-go-reports-last_updated-as-year-0001) | Minor | Pending | 1.5 = 3×1.0÷2 | mechanical | Found while writing #530's tests. 424 of 524 live queue rows serve `0001-01-01T00:00:00Z`, because `parseAssistedTime` has no layout for `time.Time`'s default string form, which is what the code writes. One layout string fixes it. |
...
**Found 2026-08-07** while writing #530's scan-alignment test, which failed on an assertion that turned out to be encoding this defect rather than a misalignment.

**The observation, live:** `/api/assisted` serves `"last_updated": "0001-01-01T00:00:00Z"` for **424 of 524** queue rows. The remaining 100 carry real timestamps.

**Cause.** `parseAssistedTime` (`pkg/storage/assisted.go`) tries four layouts: `time.RFC3339Nano`, `time.RFC3339`, `2006-01-02 15:04:05.999999999-07:00`, and `2006-01-02 15:04:05`. None matches `time.Time`'s default `String()` form — `2026-08-06 14:57:49.676108047 +0000 UTC` — which is exactly what the Go writers put in the column, because they pass a `time.Time` straight to `db.Exec` (`UpdateFunnelStatusInvalid`, `UpdateFunnelStatusWithReasonAndScore` and friends all do this). On a parse miss the function returns the zero `time.Time`, so the failure is silent. The 100 rows that work are the ones whose timestamp came from SQLite's own `CURRENT_TIMESTAMP`, which does match the fourth layout.

Verified directly: the three forms tested against those four layouts give `false` for the Go form and `true` for the RFC3339 and SQLite forms.

**Fix direction.** Add `2006-01-02 15:04:05.999999999 -0700 MST` to the layout list — one string, and the same 424 rows start reporting real times. Two things worth doing alongside it, neither strictly required: consider whether a parse miss should be logged rather than silently returning zero, and note that the same storage convention makes SQLite's own `date()`/`datetime()` return NULL on these columns, so any future SQL that filters on `date(last_updated)` will silently match nothing. That second point is a live trap — it produced a wrong "0 rows updated" reading during #524's verification before being caught.

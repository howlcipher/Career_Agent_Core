...
- [x] `go build ./...` succeeds clean — re-verified 2026-07-27 after #121
- [x] `go vet ./...` reports no issues — re-verified 2026-07-27 after #121
- [x] `go test ./...` passes for every package that has tests — **re-verified 2026-07-27 after #121:** the uncached suite is green; the race suite for `pkg/security`, `cmd/agent`, and `pkg/submitter` also passes.
- [x] A working local Ollama install with the models `cmd/agent` needs — re-verified 2026-07-27 after #121 through the live tags endpoint: `qwen3:30b-instruct`, `qwen2.5vl:7b`, and `nomic-embed-text` are present, along with `qwen3:4b-instruct`. Note: this box's original wording (`llama3.1`, `llava`, `nomic-embed-text`) is stale; `.env.example` carries the current recommendations.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **4 remain after #121 was resolved on 2026-07-27.** The application sweep filed #118-#129 and reopened the unresolved half of #112. The earlier 2026-07-24 evidence remains valid historically, but the gate is now open again.
...
**Status after #121 resolution: UNMET.** Build, vet and the full test suite are clean, but 4 open Blocker/Major defects remain. Bug work continues to outrank `improvements.md`.

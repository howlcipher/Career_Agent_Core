...
- [x] `go test ./...` passes for every package that has tests — **re-verified 2026-07-27 after #126:** all packages green, including the dashboard address, exposure-warning, handler, and timeout regressions.
...
- [x] `cmd/dashboard` serves and displays live, correct data from a populated `applications.db` — **re-verified 2026-07-27 after #126:** rebuilt and restarted the existing container dashboard, then confirmed the root page and `/api/metrics` both return HTTP 200 without printing response data. `ss` shows only `127.0.0.1:8080`, and a connection through the host's non-loopback address is refused.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **8 remain after #126 was resolved on 2026-07-27.** The application sweep filed #118-#129 and reopened the unresolved half of #112. The earlier 2026-07-24 evidence remains valid historically, but the gate is now open again.
...
**Status after #126 resolution: UNMET.** Build, vet and the full test suite are clean, but 8 open Blocker/Major defects remain. Bug work continues to outrank `improvements.md`.

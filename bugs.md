...
- [x] `go build ./...` succeeds clean — re-verified 2026-07-27 after #122
- [x] `go vet ./...` reports no issues — re-verified 2026-07-27 after #122
- [x] `go test ./...` passes for every package that has tests — **re-verified 2026-07-27 after #122:** the uncached suite is green; focused race suites for the security, scraper, submitter, and agent packages also pass.
- [x] A working local Ollama install with the models `cmd/agent` needs — re-verified 2026-07-27 after #122 through the live tags endpoint: `qwen3:30b-instruct`, `qwen2.5vl:7b`, and `nomic-embed-text` are present, along with `qwen3:4b-instruct`. Note: this box's original wording (`llama3.1`, `llava`, `nomic-embed-text`) is stale; `.env.example` carries the current recommendations.
...
- [ ] Zero open bugs below tagged `Blocker` or `Major` in the Ranked Backlog — **2 remain after #122 was resolved on 2026-07-27.** The application sweep filed #118-#129 and reopened the unresolved half of #112. The earlier 2026-07-24 evidence remains valid historically, but the gate is now open again.
...
**Status after #122 resolution: UNMET.** Resolver-bound HTTP and browser networking is verified, but bugs #128 and #112 remain open Major defects. Bug work continues to outrank `improvements.md`.
...
| 130 | [Yahoo fallback drops discovery on transient unexpected EOF responses](#130-yahoo-fallback-drops-discovery-on-transient-unexpected-eof-responses) | Major | Pending | 2.0 = 6×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | The live log recorded 148 Yahoo request failures with `unexpected EOF` in one continuous run. `discoverWithYahooHTML` returns immediately on the transport error, so each failed role/ATS query loses its discovery opportunity and receives no bounded retry or source-health signal |
| 131 | [ATS board polling discards truncated JSON without retry](#131-ats-board-polling-discards-truncated-json-without-retry) | Minor | Pending | 1.67 = 5×1.0÷3 | claude-sonnet-4-6 | gemini-3.6-flash-high | Seven `unexpected end of JSON input` responses affected four known Greenhouse/Lever boards. `pollBoard` logs the parse error and skips the entire board, so a transient or rate-limited partial response silently loses all current postings from that company for the pass |

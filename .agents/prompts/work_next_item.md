...
- Pick the delegate from what is live right now, matched to the item's `Tier` column (`mechanical` / `standard` / `deep-reasoning` — improvement #456; `documentation/model_allowlist.md` has reference IDs, but re-check live availability, it is not mechanically re-verified):

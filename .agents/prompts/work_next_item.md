...
- If the gate is `MET`, compare the open, above-floor rows in `bugs.md` and `improvements.md` as one free queue, then pick the highest-scoring item by their shared formula. A `MET` gate means no Blocker or Major bug remains; it does not make open Minor bugs ineligible. If `bugs.md` has no open rows, this naturally selects from `improvements.md`. **Never read from `improvements_paywall.md`** — it holds items that need a paid signup/subscription/API key, kept out of this prompt's default scope on purpose. Only work an item from it if the user explicitly names it in the current session.
...
- Pick the delegate from what is live right now, matched to the item's `Tier` column (`mechanical` / `standard` / `deep-reasoning` — improvement #456). `documentation/model_allowlist.md`'s "Tier → model starting point" table names a concrete model per tier per provider; re-check live availability before trusting it, it is not mechanically re-verified. If nothing at the row's tier is available, step down one tier rather than blocking:
